...
- **2026-09-24 10:18** — 进度 **934/960（97.3%）**：**M=10 四档全部 120/120 完成**（480/480）；
  M=20 PMix000/025/075 各 120/120 完成（360/360）；仅 **PMix100 M20 = 94/120** 未完成
  （DTLZ2 20、DTLZ4 20、WFG1 20 已完成；DTLZ7 14、WFG3 10、WFG8 10）。剩 26 跑。
  健康：最新 .mat 10:17:29（<2 min）；**MATLAB 进程降至 12 = 6 活跃（643 MB–1.72 GB）+ 6 个
  CPU=0 stub**，恰好对应最后 6 个在飞切片（parts 7/10/15 各 2 块），parts 2/4/8 已完成退出
  ⇒ 池已进入最后一波、无新片可发。`[done]` 1868 行 ÷2 = **934**，与文件数逐一致（互证通过）。
  无 .tmp.mat / not ok / error / timeout / poison / rc_* / DRIVER_DONE；driver.log 114 行、
  末行 09:48:44 `start PMix100 M20 part 15 runs 11-20`，driver 存活等收尾。
  **ETA ≈ 10:38–10:50**（各在飞块剩 3–5 跑 × 250–450 s；WFG 系已降至 ~250–300 s/跑，
  因 6 路并发不再超订 12 核）。吞吐 04:00→10:18 为 654 跑/6.3 h ≈ **104 跑/h**。
  口径补充：driver.log 有 **2 条 `driver start`**（00:57 预热 + 01:07 正式重启）与 **8 条非标准
  窗口 start**（`runs 2-12 / 13-20 / 2-11 / 12-20`，属重启后按缺失前缀续跑；同 run 同种子覆盖写，
  不产生重复有效 run）⇒ start 行 110 条 > 96 切片属正常，勿误判为异常重派。

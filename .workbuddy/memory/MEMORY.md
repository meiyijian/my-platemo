...
## 现在在哪（截至 2026-09-20 23:55）
- 🟡 **FE500 / M=20 / D=30 七算法扫描 —— 实验文件已建好并跑过一轮，23:52 用户要求暂停**。规格：REMO / PCSAEA / CSEA / HES_EA / SSDE / SAMOEATL2M / PACDIS × 16 题 × **20 跑**，maxFE=**500**、D=30（WFG2/3 实际 31）、N=100、每题算 IGDp；落盘 `D:\REMOandDREMO测试集\20目标\FE500\<算法>\`；种子 **22260912 + 1000×题号 + run**（与既有 FE300 批次同套种子，可逐跑配对）。框架 `PlatEMO/Experiments/FE500_M20_SevenAlgs/`（registry 为唯一真源；`driver.sh` 幂等可重驱带 `SLICE_TIMEOUT`）。**用户明确：PCSAEA / SAMOEATL2M / HES_EA 一律用原版类，不用 `_N100` 变体**。已落盘：SSDE 32/32（runs 19,20 全 16 题）、SAMOEATL2M 5、HES_EA 1，全部有效可复用。恢复：`cd FE500_M20_SevenAlgs && ALGS="CSEA PCSAEA REMO PACDIS HES_EA SAMOEATL2M SSDE" bash driver.sh`。实测单跑（M=20/FE500）：**SSDE ≈2–5 s、SAMOEATL2M ≈110 s、HES_EA ≈628 s**（末次 FE 500–543）。⚠️ 三个 `11D-1` 型算法（PCSAEA/HES_EA/SAMOEATL2M）初始就吃 329/500 FE，只剩 ~171 给搜索，**报数必须交代**。⚠️ 原以为 HES_EA 在 M=20 必死锁，**实测 DTLZ2/run19 正常跑完**（628 s、IGD 1.40→1.02）⇒ 死锁是题×种子相关的，单样本不能排除，`SLICE_TIMEOUT` 必须留。共享 harness 已加 `FESlack`（默认 0 = 旧行为不变）：**末次 FE 严禁严格判等 maxFE**（按批评估的算法必超支），否则 driver 永不收敛。

...
- 🟢 **FE500 / M=20 / D=30 七算法全系列 —— 09-21 00:00 已重新启动全量（14 路，用户要求"开最大进程"）**，后台任务 `ofzUVI`，预计 **≈14 h**（约 09-21 14:00 结束）。顺序 `SSDE SAMOEATL2M CSEA PCSAEA REMO PACDIS HES_EA`（便宜→贵；有卡死风险的 HES 放最后）。规格：REMO / PCSAEA / CSEA / HES_EA / SSDE / SAMOEATL2M / PACDIS × 16 题 × **20 跑**，maxFE=**500**、M=20、D=30（WFG2/3 实际 31）、N=100、每题算 IGDp；落盘 `D:\REMOandDREMO测试集\20目标\FE500\<算法>\`；种子 **22260912 + 1000×题号 + run**。**用户明确：PCSAEA / SAMOEATL2M / HES_EA 一律用原版类**。框架 `PlatEMO/Experiments/FE500_M20_SevenAlgs/`（registry 唯一真源；`driver.sh` 幂等可重驱）。实测单跑：SSDE ≈2–5 s、SAMOEATL2M ≈110 s、HES_EA ≈628 s。进度看 `logs/driver.log` 与 `logs/<KEY>/driver.log`，数据看 `FE500\<算法>\*.mat` 个数（满 320）。
- 🔴 **FE500 的两个致命坑（已修）**：① **末次 FE 严禁严格判等 maxFE**——按批评估的算法必超支（实测 SSDE 500–543），旧判据会让可重驱 driver 永久重算；共享 harness 已加 `FESlack`（默认 0 = 旧行为不变）。② **固定种子下卡死的 run 重驱必定再卡**——已加"投毒"机制（`mark_poison.py` + `rc_part*.txt` + `poison.txt`；同一片被 timeout 124 杀 ≥2 次即把**列表里第一个没落盘的 run** 投毒并跳过）；`SLICE_TIMEOUT` 默认 14400 s。⚠️ 三个 `11D-1` 型算法（PCSAEA/HES_EA/SAMOEATL2M）初始就吃 329/500 FE，只剩 ~171 给搜索，**报数必须交代**。

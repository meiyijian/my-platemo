...
- 🟢 **FE500 / M=20 / D=30 七算法全系列 —— 09-21 00:38 起 12 路跑**（用户 00:21 拍板：**回 12 路、关内存闸**——12 是本机验证过的值），后台任务 `Of9LG9`，预计 **≈16 h**。顺序 `SSDE SAMOEA CSEA PCSAEA REMO PACDIS HES_EA`（便宜→贵；有卡死风险的 HES 放最后）。**SSDE 已 320/320 完成**。规格：7 算法 × 16 题 × **20 跑** = 2240，maxFE=**500**、M=20、D=30（WFG2/3 实际 31）、N=100、每题算 IGDp；落盘 `D:\REMOandDREMO测试集\20目标\FE500\<算法>\`；种子 **22260912 + 1000×题号 + run**。**用户明确：PCSAEA / SAMOEATL2M / HES_EA 一律用原版类**。框架 `PlatEMO/Experiments/FE500_M20_SevenAlgs/`。**巡检自动化 `852747ad-1eec-4afc-a421-6ca505c29ed6`（每小时，有效到 09-21 20:00）**。实测单跑 wall：SSDE ≈2–5 s（DTLZ7 约 90–130 s）、SAMOEATL2M ≈110 s、HES_EA ≈628 s。

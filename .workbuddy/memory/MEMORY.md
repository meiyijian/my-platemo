...
## 现在在哪（截至 2026-09-21 00:02）
- 🟢 **FE500 / M=20 / D=30 七算法全系列 —— 09-21 00:00 起跑，00:15 修好节流后 14 路全速跑**，后台任务 `Jv5deY`，预计 **≈14 h**。顺序 `SSDE SAMOEATL2M CSEA PCSAEA REMO PACDIS HES_EA`（便宜→贵；有卡死风险的 HES 放最后）。规格：7 算法 × 16 题 × **20 跑** = 2240，maxFE=**500**、M=20、D=30（WFG2/3 实际 31）、N=100、每题算 IGDp；落盘 `D:\REMOandDREMO测试集\20目标\FE500\<算法>\`；种子 **22260912 + 1000×题号 + run**。**用户明确：PCSAEA / SAMOEATL2M / HES_EA 一律用原版类**。框架 `PlatEMO/Experiments/FE500_M20_SevenAlgs/`。**巡检自动化 `852747ad-1eec-4afc-a421-6ca505c29ed6`（每小时，有效到 09-21 20:00）**；进度看 `logs/driver.log` 与 `logs/<KEY>/driver.log`，数据看 `FE500\<算法>\*.mat` 个数（满 320）。实测单跑：SSDE ≈2–5 s、SAMOEATL2M ≈110 s、HES_EA ≈628 s。
- 🔴 **FE500 的四个致命坑（都已修）**：① **末次 FE 严禁严格判等 maxFE**——按批评估的算法必超支（实测 SSDE 500–543），旧判据会让可重驱 driver 永久重算；共享 harness 已加 `FESlack`（默认 0 = 旧行为不变）。② **固定种子下卡死的 run 重驱必定再卡**——已加"投毒"机制（`mark_poison.py` + `rc_part*.txt` + `poison.txt`；同一片被 timeout 124 杀 ≥2 次即把**列表里第一个没落盘的 run** 投毒并跳过）；`SLICE_TIMEOUT` 默认 14400 s。③ **启动节流不能用一个固定值**——`sleep 20` 是为防多进程**同时冷启动**导致 ntdll 堆损坏，但切片比 20 s 短的算法（SSDE）会被它卡到只有 2–3 路在跑、CPU 12%；改为 `LAUNCH_GAP=20`（前 `COLD_STARTS=4` 次）+ `LAUNCH_GAP_WARM=5`。④ **必须加内存闸**——本机 31.3 GB 但基线空闲仅 16.8 GB，14 路跑 patternnet/dacefit 类会换页；每次启动前查 `MIN_FREE_MB=3500`，不够就等一个切片结束（池子自动缩容，够时保持 14）。⚠️ 三个 `11D-1` 型算法（PCSAEA/HES_EA/SAMOEATL2M）初始就吃 329/500 FE，只剩 ~171 给搜索，**报数必须交代**。

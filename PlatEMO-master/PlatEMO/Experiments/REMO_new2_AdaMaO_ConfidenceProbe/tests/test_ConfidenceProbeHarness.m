...
function testValidatorAcceptsCompleteRunAndRejectsMalformedRuns(testCase)
    ...
    skippedProbe = withSkippedAuditGenerations( ...
        confidenceProbe,[1;4;9]);
    ...
    exactLagFile = fullfile(folder,'exact_h3_lag.mat');
    exactLagProbe = withSkippedAuditGenerations( ...
        confidenceProbe,[1;3;9]);
    saveRun(exactLagFile,metadata,exactLagProbe,finalPopulation, ...
        IGD,IGDp,runtime);
    [valid,message] = ValidateConfidenceProbeResultFile( ...
        exactLagFile,protocol,job);
    verifyTrue(testCase,valid,message);

    wrongObservedH3File = fullfile(folder,'wrong_observed_h3.mat');
    badProbe = setGenerationH3(skippedProbe,1,true);
    saveRun(wrongObservedH3File,metadata,badProbe,finalPopulation, ...
        IGD,IGDp,runtime);
    [valid,message] = ValidateConfidenceProbeResultFile( ...
        wrongObservedH3File,protocol,job);
    ...
    badProbe = setGenerationH3(exactLagProbe,1,false);
...
function probe = withSkippedAuditGenerations(probe,generations)
    candidateGeneration = [generations(1);generations(2); ...
    ...
    for i = 1:numel(generations)
        ...
        if ismember(generations(i)+2,generations)
            solutionBlocks{i}(:,column( ...
...
function probe = setGenerationH3(probe,generation,observed)
    tableDefinitions = { ...
        'solutionRows',{'SurviveH3','SurviveH1'}; ...
        'pbiPairRows',{ ...
            'LeftSurviveH3','LeftSurviveH1'; ...
            'RightSurviveH3','RightSurviveH1'}; ...
        'networkPairRows',{'CandidateSurviveH3','CandidateSurviveH1'}; ...
        'candidateRows',{'SurviveH3','SurviveH1'}};
    for i = 1:size(tableDefinitions,1)
        tableName = tableDefinitions{i,1};
        mappings = tableDefinitions{i,2};
        rows = probe.(tableName)(:,column( ...
            probe,tableName,'Generation')) == generation;
        for j = 1:size(mappings,1)
            target = column(probe,tableName,mappings{j,1});
            if observed
                source = column(probe,tableName,mappings{j,2});
                probe.(tableName)(rows,target) = ...
                    probe.(tableName)(rows,source);
            else
                probe.(tableName)(rows,target) = nan;
            end
        end
    end
end

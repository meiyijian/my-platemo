function analysis = analyze_ConfidenceProbe_SDE(resultDir,varargin)
%ANALYZE_CONFIDENCEPROBE_SDE Frozen v2 analysis: SDE-consistency primary.
%   The primary decision uses only good-rest PBI pairs with the SDE
%   relation as truth, so every pair is comparable by construction.
%   Strict Pareto and epsilon-Pareto (per-run min-max normalized
%   objectives, epsilon = 0.05 and 0.10) are auxiliary diagnostics and
%   are never pooled into the SDE decision. Only *_sde profiles are
%   accepted so v1 Pareto-primary result sets cannot be re-judged here.
    ...
    if ~endsWith(protocol.profile,'_sde')
        error('AdaMaO:WrongConfidenceProbeAnalyzer', ...
            ['Profile %s is not an SDE-primary (*_sde) profile; use ', ...
            'analyze_ConfidenceProbe for the frozen v1 analysis.'], ...
            protocol.profile);
    end
    if ~isfield(protocol.analysis,'epsilonLevels') || ...
            ~isequal(protocol.analysis.epsilonLevels,[0.05,0.10])
        error('AdaMaO:InvalidConfidenceProbeEpsilonLevels', ...
            'Profile %s does not freeze epsilonLevels = [0.05,0.10].', ...
            protocol.profile);
    end
    epsilonLevels = protocol.analysis.epsilonLevels;
    assertUniqueRuns(records);
    stream = RandStream('mt19937ar','Seed',bootstrapSeed);

    pbiPairBins = concatenateRecordTables( ...
        records,@(record)aggregatePBIPairs(record,epsilonLevels));
    ...
    runMetrics = buildRunMetrics(records,epsilonLevels);
    ...
    outputFiles = { ...
        'ConfidenceSDE_PBI_pair_bins.csv',pbiPairBins; ...
        'ConfidenceSDE_PBI_solution_bins.csv',solutionBins; ...
        'ConfidenceSDE_network_pair_bins.csv',networkPairBins; ...
        'ConfidenceSDE_candidate_bins.csv',candidateBins; ...
        'ConfidenceSDE_summary_by_problem.csv',summaryByProblem; ...
        'ConfidenceSDE_summary_by_M.csv',summaryByM; ...
        'ConfidenceSDE_decision.csv',decision};
    ...
    analysis.primaryTruth = 'SDE';
    analysis.epsilonLevels = epsilonLevels;
    ...
    save(fullfile(outputDir,'ConfidenceSDE_analysis.mat'), ...
        'analysis','-v7.3');
end
...
function [records,protocol] = loadRecords(resultDir)
    ...
    records = repmat(struct( ...
        'file','','metadata',struct(),'probe',struct(), ...
        'normalizedObjs',zeros(0,0)),numel(files),1);
    protocol = [];
    for i = 1:numel(files)
        ...
        records(i).normalizedObjs = normalizedArchiveObjectives( ...
            loaded.finalPopulation,loaded.metadata.maxFE);
    end
end

function normalized = normalizedArchiveObjectives(population,maxFE)
%   Rebuild the EvalID-ordered archive exactly as the validator does,
%   then min-max normalize each objective over all evaluated solutions.
    ...
    elseif isstruct(population) && isfield(population,'adds')
        ...
        error('AdaMaO:MissingConfidenceProbeArchive', ...
            'finalPopulation does not expose EvalID additions.');
    end
    evalIDs = evalIDs(:);
    [sortedIDs,order] = sort(evalIDs);
    if ~isequal(sortedIDs,(1:maxFE)')
        error('AdaMaO:MissingConfidenceProbeArchive', ...
            'final Archive EvalID values are not continuous 1:maxFE.');
    end
    objectives = objectives(order,:);
    lowerBound = min(objectives,[],1);
    range = max(objectives,[],1) - lowerBound;
    range(range <= 0) = 1;
    normalized = (objectives - lowerBound)./range;
end

function relation = epsilonRelations(normalizedObjs,leftID,rightID, ...
    epsilon)
%   Additive epsilon-dominance on normalized objectives. Mutual or
%   absent epsilon-dominance both map to 0 (excluded like incomparable).
    zLeft = normalizedObjs(leftID,:);
    zRight = normalizedObjs(rightID,:);
    leftDominates = all(zLeft <= zRight + epsilon,2);
    rightDominates = all(zRight <= zLeft + epsilon,2);
    relation = double(leftDominates & ~rightDominates) - ...
        double(rightDominates & ~leftDominates);
end
...
function result = aggregatePBIPairs(record,epsilonLevels)
    schema = record.probe;
    ...
    leftID = getColumn(rows,schema,'pbiPairRows','LeftEvalID');
    rightID = getColumn(rows,schema,'pbiPairRows','RightEvalID');
    eps005 = epsilonRelations(record.normalizedObjs, ...
        leftID,rightID,epsilonLevels(1));
    eps010 = epsilonRelations(record.normalizedObjs, ...
        leftID,rightID,epsilonLevels(2));
    ...
    Eps005ComparableN = zeros(count,1);
    Eps005ErrorRate = nan(count,1);
    Eps010ComparableN = zeros(count,1);
    Eps010ErrorRate = nan(count,1);
    ...
    for g = 1:count
        ...
        [Eps005ComparableN(g),Eps005ErrorRate(g)] = relationRates( ...
            predicted(mask),eps005(mask));
        [Eps010ComparableN(g),Eps010ErrorRate(g)] = relationRates( ...
            predicted(mask),eps010(mask));
    ...
    result = table(Problem,M,Run,Seed,keyRows.Generation, ...
        ...
        Eps005ComparableN,Eps005ErrorRate, ...
        Eps010ComparableN,Eps010ErrorRate, ...
...
function runMetrics = buildRunMetrics(records,epsilonLevels)
    count = numel(records);
    ...
    PBIParetoQ5MinusQ1Error = nan(count,1);
    PBIParetoAUROC = nan(count,1);
    PBIParetoComparableN = zeros(count,1);
    Eps005Q5MinusQ1Error = nan(count,1);
    Eps005AUROC = nan(count,1);
    Eps005ComparableN = zeros(count,1);
    Eps010Q5MinusQ1Error = nan(count,1);
    Eps010AUROC = nan(count,1);
    Eps010ComparableN = zeros(count,1);
    ...
    for i = 1:count
        ...
        pbi = pbiRunMetrics(record,epsilonLevels);
        Q1ErrorRate(i) = pbi.sde.q1;
        Q5ErrorRate(i) = pbi.sde.q5;
        Q5MinusQ1Error(i) = pbi.sde.q5-pbi.sde.q1;
        AUROC(i) = pbi.sde.auc;
        ComparablePairN(i) = pbi.sde.comparableCount;
        PrimaryJointValid(i) = all(isfinite([ ...
            Q1ErrorRate(i),Q5ErrorRate(i), ...
            Q5MinusQ1Error(i),AUROC(i)]));
        PBIParetoQ5MinusQ1Error(i) = pbi.pareto.q5-pbi.pareto.q1;
        PBIParetoAUROC(i) = pbi.pareto.auc;
        PBIParetoComparableN(i) = pbi.pareto.comparableCount;
        Eps005Q5MinusQ1Error(i) = pbi.eps005.q5-pbi.eps005.q1;
        Eps005AUROC(i) = pbi.eps005.auc;
        Eps005ComparableN(i) = pbi.eps005.comparableCount;
        Eps010Q5MinusQ1Error(i) = pbi.eps010.q5-pbi.eps010.q1;
        Eps010AUROC(i) = pbi.eps010.auc;
        Eps010ComparableN(i) = pbi.eps010.comparableCount;
    ...
    runMetrics = table(Problem,M,Run,Seed,Q1ErrorRate,Q5ErrorRate, ...
        ...
        PrimaryJointValid,PBIParetoQ5MinusQ1Error,PBIParetoAUROC, ...
        PBIParetoComparableN,Eps005Q5MinusQ1Error,Eps005AUROC, ...
        Eps005ComparableN,Eps010Q5MinusQ1Error,Eps010AUROC, ...
        Eps010ComparableN,NetworkParetoQ5MinusQ1Error, ...
...
function metrics = pbiRunMetrics(record,epsilonLevels)
%   All truths share the same good-rest rows, confidence values, and
%   generation-stratified quintile bins; only the truth column differs.
    schema = record.probe;
    rows = schema.pbiPairRows;
    pairType = getColumn(rows,schema,'pbiPairRows','PairType');
    ...
    eps010 = epsilonRelations(record.normalizedObjs, ...
        leftID,rightID,epsilonLevels(2));
    metrics = struct();
    metrics.sde = truthSummary(confidence,predicted,sde,bins);
    metrics.pareto = truthSummary(confidence,predicted,pareto,bins);
    metrics.eps005 = truthSummary(confidence,predicted,eps005,bins);
    metrics.eps010 = truthSummary(confidence,predicted,eps010,bins);
end

function summary = truthSummary(confidence,predicted,truth,bins)
    comparable = truth ~= 0;
    error = predicted ~= truth;
    summary = struct();
    summary.q1 = conditionalMean(error,bins == 1 & comparable);
    summary.q5 = conditionalMean(error,bins == 5 & comparable);
    summary.comparableCount = sum(comparable);
    summary.auc = binaryAUC(confidence(comparable),~error(comparable));
end
...
function [paretoDifference,paretoAUC,paretoComparableCount, ...
    ...
    bins = AssignConfidenceProbeBins(confidence, ...
        table(generation,predicted),5);
    pareto = getColumn(rows,schema,'networkPairRows','ParetoRelation');
    paretoSummary = truthSummary(confidence,predicted,pareto,bins);
    paretoDifference = paretoSummary.q5-paretoSummary.q1;
    paretoAUC = paretoSummary.auc;
    paretoComparableCount = paretoSummary.comparableCount;
    sde = getColumn(rows,schema,'networkPairRows','SDERelation');
    sdeSummary = truthSummary(confidence,predicted,sde,bins);
    sdeDifference = sdeSummary.q5-sdeSummary.q1;
    sdeAUC = sdeSummary.auc;
    sdeComparableCount = sdeSummary.comparableCount;
end
...
function summary = addProblemComparableCounts(summary,runMetrics)
    columns = {'PBIParetoComparableN','Eps005ComparableN', ...
        'Eps010ComparableN','NetworkParetoComparableN', ...
...
function specs = auxiliaryMetricSpecs()
    definitions = { ...
        'PBIParetoQ5MinusQ1Error','PBIParetoQ5MinusQ1Error', ...
            'PBIParetoDiff',-1,'PBIParetoFavorableProblemCount'; ...
        'PBIParetoAUROC','PBIParetoAUROC','PBIParetoAUROC',0,''; ...
        'Eps005Q5MinusQ1Error','Eps005Q5MinusQ1Error', ...
            'Eps005Diff',-1,'Eps005FavorableProblemCount'; ...
        'Eps005AUROC','Eps005AUROC','Eps005AUROC',0,''; ...
        'Eps010Q5MinusQ1Error','Eps010Q5MinusQ1Error', ...
            'Eps010Diff',-1,'Eps010FavorableProblemCount'; ...
        'Eps010AUROC','Eps010AUROC','Eps010AUROC',0,''; ...
...
function decision = makeDecision(summary)
    ...
    for i = 1:height(summary)
        ...
        if ~PrimaryGatePassed(i)
            ...
            Interpretation(i) = ...
                "The frozen SDE-consistency confidence gate was not passed.";
        elseif AbsoluteErrorReduction(i) >= 0.05
            DecisionCode(i) = "GATE_DEVELOPMENT_VALUE";
            Interpretation(i) = ...
                "PBI confidence passed the SDE gate with at least " + ...
                "5pp absolute error reduction.";
        else

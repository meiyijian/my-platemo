...
%GOODGROUPPRECISIONTEST Contract, metric, persistence, and smoke tests.
    ...
    methods (Test)
        function formalProtocolIsExact(testCase)
            ...
            testCase.verifyNumElements(jobs, 500);
            testCase.verifyEqual(unique([jobs.M]), [10, 20]);
            testCase.verifyEqual(unique([jobs.RequestedD]), 30);
            testCase.verifyEqual(unique([jobs.ExpectedActualD]), [30, 31]);
            testCase.verifyTrue(all( ...
                [jobs([jobs.Problem] == "WFG3").ExpectedActualD] == 31));
            testCase.verifyTrue(all( ...
                [jobs([jobs.Problem] ~= "WFG3").ExpectedActualD] == 30));
            testCase.verifyEqual(unique([jobs.MaxFE]), 500);
            testCase.verifyEqual(unique([jobs.Run]), 1:25);
            testCase.verifyEqual(numel(unique([jobs.Seed])), 500);
            testCase.verifyEqual([config.Problems.Name], ...
            ...
            testCase.verifyEqual(GGPStableSeed(5, 20, 25), 52025);
            testCase.verifyEqual(GGPStableSeed(10, 20, 25), 102025);
        end

        function originalProtocolRemainsAvailable(testCase)
            config = GGPProtocol("formal", "original");

            testCase.verifyNumElements(config.Jobs, 250);
            testCase.verifyEqual([config.Problems.Name], ...
                ["DTLZ2", "DTLZ4", "DTLZ7", "WFG3", "WFG7"]);
            testCase.verifyEqual(numel(unique([config.Jobs.Seed])), 250);
        end
        ...
        function analysisUsesRunStageUnits(testCase)
            ...
            testCase.verifyEqual( ...
                string(outputs.CheckpointMetrics.Properties.VariableNames), ...
                ["SchemaVersion", "Profile", "Problem", "M", ...
                "RequestedD", "ActualD", "Run", "Seed", "SnapshotID", ...
                "Generation", "FE", "FERatio", "Stage", "View", ...
                "SelectionRule", "Truth", "TruthType", "Horizon", ...
                "Censored", "PopulationSize", "SelectedCount", ...
                "SelectedRate", "TruthPositiveCount", "TruePositiveCount", ...
                "Precision", "PrecisionAt25", "NativePrecision", "Recall", ...
                "Chance", "Lift", "AUC", "RetentionRate"]);
            testCase.verifyEqual( ...
                string(outputs.PerRunStage.Properties.VariableNames), ...
                ["Problem", "M", "Run", "Seed", "Stage", "View", ...
                "SelectionRule", "Truth", "TruthType", "Horizon", ...
                "NumberOfCheckpoints", "ValidPrecisionCheckpoints", ...
                "MeanSelectedRate", "MeanPrecision", "MeanRecall", ...
                "MeanChance", "MeanLift", "MeanAUC", "MeanRetentionRate"]);
            testCase.verifyEqual( ...
                string(outputs.PairedComparisons.Properties.VariableNames), ...
                ["Problem", "M", "Stage", "Truth", "Metric", "ViewA", ...
                "ViewB", "AvailablePairedRuns", "ValidPairs", "MeanA", ...
                "MeanB", "MeanDelta", "MedianDelta", ...
                "MeanRelativeImprovementPct", "PValueRaw", ...
                "PairedWinProbability", "RankBiserial", "PValueHolm", ...
                "RejectHolm05"]);
            testCase.verifyEqual( ...
                string(outputs.Coverage.Properties.VariableNames), ...
                ["Problem", "M", "ExpectedRuns", "ObservedRuns", ...
                "MissingRuns", "Complete"]);
        end

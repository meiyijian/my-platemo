function merge_IGDp_M20(maxFiles)
%MERGE_IGDP_M20 Append the IGD+ trace to every M=20 NoBatchDist raw result file.
...
%     2. the stored IGD is re-derived for the first and last snapshot and
%        compared with `metric.IGD`; a mismatch larger than the tolerance
%        aborts the whole pass, which proves the snapshot-to-value mapping
%        is right (checking all 30 would only double the cost);
...
%     5. files are processed one at a time: bulk concurrent MATLAB writes
%        have previously been linked to heap corruption on this machine.
%
%   merge_IGDp_M20(maxFiles) limits the work to the first maxFiles result
%   files, for a dry trial before the full pass.
%
%   Scope: ONLY the 20-objective NoBatchDist dataset. M=10/M=15 untouched.

    if nargin < 1, maxFiles = inf; end

    root  = 'D:\REMOandDREMO测试集\20目标\REMO_UniformMix_Pruned_Weighted_Lambdat030_NoBatchDist';
    ...
    for p = 1:numel(probs)
        ...
        for r = 1:20
            ...
            if relIGD > tol
                ...
                nBadIGD = nBadIGD + 1;
                error('MergeIGDpM20:igdMismatch', ...
            ...
            if isfield(S.metric,'IGDp') && isequal(S.metric.IGDp(:), igdpTrace)
                ...
                if ~isequal(V.metric.IGDp(:), igdpTrace)
                    error('MergeIGDpM20:verifyFail', 'IGDp mismatch after write: %s', file);
                end
                nWritten = nWritten + 1;
            end
    ...
    writetable(T,fullfile(outDir,'IGDp_M20_perrun.csv'));

function checkpointMetrics = LTGGPComputeRunMetrics(snapshots, trajectory, evaluations, metadata)
%LTGGPCOMPUTERUNMETRICS Score label views against stable-ID future outcomes.
%   CHECKPOINTMETRICS = LTGGPCOMPUTERUNMETRICS(SNAPSHOTS, TRAJECTORY,
...
%   manufacture a Top-25% group.
    ...
    if isempty(snapshots) || isempty(trajectory)
        error("LTGGP:EmptyAuditTrajectory", ...
    ...
    for snapshotIndex = 1:numel(snapshots)
        ...
        if numel(unique(populationIDs)) ~= populationSize || ...
            ...
            error("LTGGP:InvalidEvaluationIdentity", ...
        ...
        stage = LTGGPStageBin(snapshot.Ratio);

        for viewIndex = 1:numel(views)
            view = views(viewIndex);
            for truthIndex = 1:numel(truths)
                truth = truths(truthIndex);
                metric = LTGGPBinaryMetrics(view.Selected, truth.Values, view.Score);
                rowIndex = rowIndex + 1;
                row = emptyRow;
...
function views = makeViews(snapshot, rGood)
    ...
    if ~isequal(hybridTop, catalogCurrent)
        error("LTGGP:HybridCatalogMismatch", ...
...
function selected = selectTopFraction(score, fraction)
    ...
    if any(~isfinite(score))
        error("LTGGP:InvalidViewScore", "View scores must be finite.");
    end

function outputs = analyze_Lambdat030GoodGroupPrecision(profile, varargin)
%ANALYZE_LAMBDAT030GOODGROUPPRECISION Analyze completed per-run LTGGP files.
%   OUTPUTS = ANALYZE_LAMBDAT030GOODGROUPPRECISION(PROFILE) aggregates
%   checkpoints within each run and FE stage before any statistical
%   comparison. It reports paired Wilcoxon tests, paired effect sizes,
%   relative improvement, and Holm-adjusted p-values.
%
%   Name-value option:
...
%   summary because its selected-set size is not fixed at 25%.
    ...
    if isempty(rawFiles)
        error("LTGGP:NoRawFiles", "No per-run MAT files were found in %s.", rawDirectory);
    end
    ...
    for fileIndex = 1:numel(rawFiles)
        ...
        [isValid, report] = LTGGPValidateRunFile(filePath, profile);
        if ~isValid
            error("LTGGP:InvalidRawFile", "Invalid raw file %s: %s", filePath, report.Detail);
        end
    ...
    outputPaths = struct( ...
        "CheckpointMetrics", fullfile(analysisDirectory, "LTGGP_CheckpointMetrics.csv"), ...
        "PerRunStage", fullfile(analysisDirectory, "LTGGP_PerRunStage.csv"), ...
        "PairedComparisons", fullfile(analysisDirectory, "LTGGP_PairedComparisons.csv"), ...
        "NativeLabelSummary", fullfile(analysisDirectory, "LTGGP_LabelDynNative.csv"), ...
        "Coverage", fullfile(analysisDirectory, "LTGGP_Coverage.csv"));
    ...
    if any(~coverage.Complete)
        warning("LTGGP:IncompleteCoverage", ...
...
function comparisons = buildPairedComparisons(perRunStage)
    ...
    for family = 1:max(familyIndex)
        ...
        comparisons.PValueHolm(members) = ...
            LTGGPHolmAdjust(comparisons.PValueRaw(members));
    end

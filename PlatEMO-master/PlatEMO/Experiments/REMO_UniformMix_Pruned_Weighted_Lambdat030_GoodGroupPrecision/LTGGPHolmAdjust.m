function adjustedP = LTGGPHolmAdjust(rawP)
%LTGGPHOLMADJUST Apply Holm's step-down correction to a vector of p-values.
%   NaN entries remain NaN and are excluded from the correction family.
    ...
    if any(rawP(valid) < 0 | rawP(valid) > 1)
        error("LTGGP:InvalidPValue", "Finite p-values must lie in [0,1].");
    end

...
%   Ref 为从当前种群选出的已评价参考解，同时用于后续交配池。
    ...
    [label_dyn,posRate,delta,normG] = PWRepresentativeAudit(PopObj, RefObj);
    ...
    DirectionSource = 0; FallbackReason = 'Weighted production';
    Front1Count = sum(NDSort(PopObj,1)==1); ClusterCount = 0;
    views = struct();
    views.V                  = V;

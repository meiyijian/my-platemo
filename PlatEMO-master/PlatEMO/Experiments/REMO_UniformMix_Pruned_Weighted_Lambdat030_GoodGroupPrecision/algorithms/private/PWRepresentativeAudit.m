function [Output,r,deltaUsed,normG] = PWRepresentativeAudit(varargin)
...
%--------------------------------------------------------------------------
    ...
    if selfadapt
        ...
        while r>0.7 || r<0.3
            ...
            [l,r,normG] = split_data(Pop,Ref,delt_c); deltaUsed = delt_c;
            if r > 0.7
        ...
        [l,r,normG] = split_data(Pop,Ref,delt); deltaUsed = delt;
    end
    Output = l;
end
%% ============ 内部函数：基于 PBI 的划分 ============
function [Output,rate,normG] = split_data(Pop,Ref,delt)

...
# the driver can relaunch only the gaps at good load balance. Runs are chunked
# (default 10 per slice) because a 16-problem x 20-run grid with a 12-process
# pool would otherwise leave four full-problem slices (about 2.3 h each)
# waiting behind the first wave.
# Problem order is the paper's 16-problem list: 1..7 = DTLZ1..7, 8..16 = WFG1..9.
import os

ROOT = r"D:\REMOandDREMO测试集\10目标\n30\REMO_UniformMix_Pruned_Weighted_Lambdat030_NoBatchDist"
...
for index, problem in enumerate(PROBS, 1):
    ...
    for run in range(1, 21):
        found = any(os.path.isfile(os.path.join(
            ROOT, "%s_%s_M10_D%d_%d.mat" % (ALG, problem, d, run))) for d in (30, 31))
        if not found:
            missing.append(run)

...
function testBuildAndPredictRejectInvalidCatalogAndEvalIDs(testCase)
    objectives = [0 1;1 0];
    validCatalog = logical([1;0]);
    confidence = [0.4;0.8];
    sdeFitness = [1;0];
    invalidCatalogs = {[1;NaN],[1;2]};

    for i = 1:numel(invalidCatalogs)
        verifyError(testCase,@() BuildSDEConfidencePairAudit( ...
            1,2,[1;2],objectives,zeros(2,0),invalidCatalogs{i}, ...
            confidence,sdeFitness),'AdaMaO:InvalidConfidenceSnapshot');
    end
    verifyError(testCase,@() BuildSDEConfidencePairAudit( ...
        1,2,[1;2.5],objectives,zeros(2,0),validCatalog, ...
        confidence,sdeFitness),'AdaMaO:InvalidConfidenceSnapshot');

    model = deterministicModel();
    for i = 1:numel(invalidCatalogs)
        verifyError(testCase,@() PredictSDEConfidenceCandidatePairs( ...
            1,2,3,0.5,[1;2],[0.25;0.75],invalidCatalogs{i},model), ...
            'AdaMaO:InvalidConfidenceCandidatePairs');
    end
    verifyError(testCase,@() PredictSDEConfidenceCandidatePairs( ...
        1,2,3.5,0.5,[1;2],[0.25;0.75],validCatalog,model), ...
        'AdaMaO:InvalidConfidenceCandidatePairs');
    verifyError(testCase,@() PredictSDEConfidenceCandidatePairs( ...
        1,2,3,0.5,[1;2.5],[0.25;0.75],validCatalog,model), ...
        'AdaMaO:InvalidConfidenceCandidatePairs');
end
...
function testCompleteRejectsMalformedNetworkRowsAndHistory(testCase)
    probe = SDEConfidenceProbeSchema();
    candidateIDs = [301;302];
    anchorIDs = [201;202];
    networkRows = PredictSDEConfidenceCandidatePairs( ...
        1,4,candidateIDs,[0.75;0.25],anchorIDs,[0.25;0.75], ...
        logical([1;0]),deterministicModel());
    errorID = 'AdaMaO:InvalidConfidenceCandidateAudit';

    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows(1:end-1,:),candidateIDs,anchorIDs),errorID);
    duplicateRows = networkRows;
    duplicateRows(end,:) = networkRows(1,:);
    verifyError(testCase,@() completeCandidateFixture( ...
        duplicateRows,candidateIDs,anchorIDs),errorID);

    leftProbability = column(probe,'networkPairRows', ...
        'ProbabilityLeftBetter');
    sameProbability = column(probe,'networkPairRows','ProbabilitySame');
    rightProbability = column(probe,'networkPairRows', ...
        'ProbabilityRightBetter');
    networkConfidence = column(probe,'networkPairRows', ...
        'NetworkConfidence');
    predictedRelation = column(probe,'networkPairRows', ...
        'PredictedRelation');
    candidateIDColumn = column(probe,'networkPairRows', ...
        'CandidateEvalID');

    outOfRange = networkRows;
    outOfRange(1,[leftProbability,rightProbability]) = [1.1,-0.1];
    verifyError(testCase,@() completeCandidateFixture( ...
        outOfRange,candidateIDs,anchorIDs),errorID);
    wrongSum = networkRows;
    wrongSum(1,sameProbability) = 0.3;
    verifyError(testCase,@() completeCandidateFixture( ...
        wrongSum,candidateIDs,anchorIDs),errorID);
    wrongConfidence = networkRows;
    wrongConfidence(1,networkConfidence) = 0.1;
    verifyError(testCase,@() completeCandidateFixture( ...
        wrongConfidence,candidateIDs,anchorIDs),errorID);
    wrongRelation = networkRows;
    wrongRelation(1,predictedRelation) = 0;
    verifyError(testCase,@() completeCandidateFixture( ...
        wrongRelation,candidateIDs,anchorIDs),errorID);
    fractionalRowID = networkRows;
    fractionalRowID(1,candidateIDColumn) = 301.5;
    verifyError(testCase,@() completeCandidateFixture( ...
        fractionalRowID,candidateIDs,anchorIDs),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,[301;302.5],anchorIDs),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,[201;202.5]),errorID);

    historyIDs = [199;201;202];
    historyObj = [9 9;0 2;2 0];
    historyCon = zeros(3,0);
    verifyWarningFree(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',historyIDs, ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon));
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs,'HistoryEvalIDs',[199;201], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',[199;200;201;202], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',[199;201;201], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',[199.5;201;202], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',[199;200;201], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',[301;201;202], ...
        'HistoryObjectives',historyObj, ...
        'HistoryConstraints',historyCon),errorID);
    mismatchedHistoryObj = historyObj;
    mismatchedHistoryObj(2,1) = 0.25;
    verifyError(testCase,@() completeCandidateFixture( ...
        networkRows,candidateIDs,anchorIDs, ...
        'HistoryEvalIDs',historyIDs, ...
        'HistoryObjectives',mismatchedHistoryObj, ...
        'HistoryConstraints',historyCon),errorID);
end

function testUpdateRejectsInvalidIDsAndNonlogicalFinalFlag(testCase)
    probe = SDEConfidenceProbeSchema();
    errorID = 'AdaMaO:InvalidConfidenceProbeUpdate';

    invalidActiveIDs = {[1;1],[1;1.5],[0;1],[-1;1]};
    for i = 1:numel(invalidActiveIDs)
        verifyError(testCase,@() UpdateSDEConfidenceProbe( ...
            probe,1,invalidActiveIDs{i},1,false),errorID);
    end
    invalidCurrentIDs = {[2;2],[1;2.5],[0;2],[-1;2]};
    for i = 1:numel(invalidCurrentIDs)
        verifyError(testCase,@() UpdateSDEConfidenceProbe( ...
            probe,1,[1;2],invalidCurrentIDs{i},false),errorID);
    end
    verifyError(testCase,@() UpdateSDEConfidenceProbe( ...
        probe,1,[1;2],1,1),errorID);
end
...
function completeCandidateFixture( ...
    networkRows,candidateIDs,anchorIDs,varargin)
    CompleteSDEConfidenceCandidateAudit( ...
        networkRows,candidateIDs,[0.5 0.5;3 3],zeros(2,0), ...
        anchorIDs,[0 2;2 0],zeros(2,0),[0.5 0.5],1,varargin{:});
end

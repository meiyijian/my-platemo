classdef REMO_new2_AdaMaO_SDEOnly_DualPBIContModeBase < ALGORITHM
% Shared runtime for continuous dual-PBI relation-mode experiments.

    methods
        function main(Algorithm,Problem)
            %% Parameters kept identical to CurrentGate
            ...
            %% Initialization kept identical to CurrentGate
            if Problem.D <= 10
                N = 11*Problem.D - 1;
            else
                N = 100;
            ...
            while Algorithm.NotTerminated(Archive)
                ...
                % Advance stochastic policies once per generation so that
                % early exits cannot shift the paired draw sequence.
                ...
                % Keep the original progress definition for score blending
                % and candidate exploration.
                ratio = Problem.FE / Problem.maxFE;
                k_eff = min(Problem.N,max(k,ceil(1.5*Problem.M)));
                [Catalog,confidence,Ref] = DualPBIContinuousSupervision( ...
                    Population,ratio,N,k_eff,5);
                ...
                relation_mode = Algorithm.relationPairMode( ...
                    prev_p_err,mean_conf,diagnostics.coverage,tau_err);
                ...
                %% Fixed SDE indicator model is trained in every variant
                IndicatorModel = [];
                if use_indicator
                    try
                ...
                %% The only experimental change: candidate-mode policy
                ...
                if debug
                    fprintf(['[AdaMaO-%s Gen %3d | FE=%4d/%4d] ', ...
                             'rel=%s cand=%s progress=%.3f Pind=%.3f u=%.3f ', ...
                             'p_err=%.3f prev=%.3f cov=%.3f deg=%.3f ', ...
                             'conf=%.3f n=%d\n'], ...
    ...
    methods (Access = protected)
        function policy = candidatePolicy(~) %#ok<STOUT>
            error('AdaMaO:MissingCandidatePolicy', ...
                'A concrete candidate-mode policy is required.');
        end

        function mode = relationPairMode( ...
                ~,previousError,meanAgreement,coverage,errorThreshold)
            mode = 'conservative';
            if previousError > errorThreshold
                mode = 'curriculum';
            elseif previousError <= errorThreshold && ...
                    meanAgreement >= 0.55 && coverage < 0.60
                mode = 'weighted';
            end
        end
    end

function [Next,info] = LambdaWeightedBatchSelection(Candidates,scores,uncertainty,qKeep,batchSize,lambda0,ratio)
%LambdaWeightedBatchSelection Quality filter followed by an ambiguity reward.
...
%       A_t(x) = R~(x) + lambda0*(1-ratio)*U~(x),  ratio = FE/maxFE.
...
%   lambda0 = 0 makes A_t an increasing affine image of R, so the batch is
%   bit-for-bit the pruned selector's batch.
...
%   departs from the same selector run with the reward switched off.
    ...
    selected = zeros(0,1);
    Next = Candidates([],:);
    lambdaT = lambda0*max(0,1-ratio);
    info = struct('nCandidates',size(Candidates,1),'nRetained',0, ...
        'batchSize',batchSize,'target',0,'lambda0',lambda0,'ratio',ratio, ...

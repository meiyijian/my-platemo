classdef REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned_Weighted_Lambda020 < ALGORITHM
% <2026> <multi/many> <real> <expensive>
% Pruned quality filter plus a time-decaying ambiguity reward.
...
%   A_t(x) = R~(x) + lambda0*(1-FE/maxFE)*U~(x)
...
% filter are NOT restored. PAQC, candidate generation, the indicator branch,
% pMix, nMax and every other setting stay identical to the pruned baseline.
...
% lambda0 --- 0.20 --- Base weight of the predicted-ambiguity reward

    methods
        function main(Algorithm,Problem)
            global ADAMAO_LAMBDA020_DIAG
            if numel(Algorithm.parameter) > 6
                error('AdaMaO:InvalidParameterCount', ...
                    'Expected at most six parameters: gmax,pMix,rGood,qKeep,nMax,lambda0.');
            end
            [gmax,pMix,rGood,qKeep,nMax,lambda0] = ...
                Algorithm.ParameterSet(3000,0.50,0.25,0.70,6,0.20);
            ...
            ADAMAO_LAMBDA020_DIAG = struct('enabled',true,'lambda0',lambda0, ...
            ...
            while Algorithm.NotTerminated(Archive)
                ADAMAO_LAMBDA020_DIAG.rounds = ADAMAO_LAMBDA020_DIAG.rounds + 1;
                u = rand(modeStream,1);
...
function validateUniformMixParameters(gmax,pMix,rGood,qKeep,nMax,lambda0)

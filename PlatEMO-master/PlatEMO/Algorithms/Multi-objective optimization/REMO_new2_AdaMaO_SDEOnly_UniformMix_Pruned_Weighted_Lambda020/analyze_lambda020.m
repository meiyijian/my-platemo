function analysis = analyze_lambda020()
%analyze_lambda020 Compare the Lambda020 runs with their matched-pair controls.
...
%   source revision, the session and the process pool with the Lambda020 runs.
...
%configuration Fixed paths and settings of this comparison.
    ...
    cfg.M = 10;
    cfg.D = 30;
    cfg.settings = ['qKeep=0.70 (relation-score quantile), nMax=6, lambda0=0.20, ' ...
...
%writeReport Markdown report with the decision-relevant numbers first.
    file = fullfile(outDir,'Lambda020_M10_10runs_分析报告.md');
    fid = fopen(file,'w','n','UTF-8');
    assert(fid >= 0,'Cannot write the report.');
    ps = struct2table(analysis.problemSummary);
    fprintf(fid,'# Lambda020（lambda0=0.20）十目标 10 次配对实验\n\n');
    ...
    if ~isempty(sst) && height(sst) > 0
        ...
        fprintf(fid,'| 问题 | n | Lambda020 均值 | 存盘对照均值 | 均值差 | 改善/变差/持平 | 符号 p |\n');
        fprintf(fid,'|---|---:|---:|---:|---:|---|---:|\n');
        for i = 1:height(sst)
    ...
    fprintf(fid,'- 有效奖励权重 lambda_t = 0.20*(1-FE/maxFE)：平均 %.4g，范围 %.4g ~ %.4g。\n', ...

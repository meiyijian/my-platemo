...
%   time-decaying reward on the predicted ambiguity inside the retained set:
%   A_t = R~ + lambda0*(1-FE/maxFE)*U~. The reward weight comes from
%   Smodel.lambda0; no p_err gate, no minimum batch completion and no second
...
%   Indicator mode uses the 30-percent relation shortlist and direct ranking.
    ...
    switch Smodel.mode
        case 'explore'
            lambda0 = 0;
            if isfield(Smodel,'lambda0') && ~isempty(Smodel.lambda0)
                lambda0 = Smodel.lambda0;
            end
            ratio = Problem.FE/Problem.maxFE;
            [Next,info] = LambdaWeightedBatchSelection( ...
                allCandidates,scores,uncertainty,qKeep,batchSize, ...
                lambda0,ratio);
            recordLambdaDiagnostics(info,Problem,nMax,size(allCandidates,1));
        case 'indicator'
...
function recordLambdaDiagnostics(info,Problem,nMax,nCandidates)
%recordLambdaDiagnostics Append one exploration record to the global collector.
...
%   cannot disturb the optimization stream.
    ...
    if ~isstruct(ADAMAO_LAMBDA020_DIAG) || ...
            ~isfield(ADAMAO_LAMBDA020_DIAG,'enabled') || ...
            ~ADAMAO_LAMBDA020_DIAG.enabled
        return;
    end
    info.FE = Problem.FE;
    info.nMax = nMax;
    info.nCandidates = nCandidates;
    ADAMAO_LAMBDA020_DIAG.iterations = ADAMAO_LAMBDA020_DIAG.iterations + 1;
    ADAMAO_LAMBDA020_DIAG.records{end+1} = info;
end

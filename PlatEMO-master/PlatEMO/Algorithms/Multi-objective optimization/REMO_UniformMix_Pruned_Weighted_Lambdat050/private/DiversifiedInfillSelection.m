...
%   A_t = R~ + 0.50*U~. The reward weight comes from
...
%   Indicator mode uses the 30-percent relation shortlist and direct ranking.
    ...
    switch Smodel.mode
        case 'explore'
            ...
            [Next,info] = Lambdat050WeightedBatchSelection( ...
...
%   cannot disturb the optimization stream.
    global ADAMAO_LAMBDAT050_DIAG
    if ~isstruct(ADAMAO_LAMBDAT050_DIAG) || ...
            ~isfield(ADAMAO_LAMBDAT050_DIAG,'enabled') || ...
            ~ADAMAO_LAMBDAT050_DIAG.enabled
        return;
    end
    info.FE = Problem.FE;
    info.nMax = nMax;
    info.nCandidates = nCandidates;
    ADAMAO_LAMBDAT050_DIAG.iterations = ADAMAO_LAMBDAT050_DIAG.iterations + 1;
    ADAMAO_LAMBDAT050_DIAG.records{end+1} = info;
end

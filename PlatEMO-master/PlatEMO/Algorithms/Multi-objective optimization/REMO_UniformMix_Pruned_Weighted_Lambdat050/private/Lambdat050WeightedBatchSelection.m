function [Next,info] = Lambdat050WeightedBatchSelection(Candidates,scores,uncertainty,qKeep,batchSize,ratio)
%Lambdat050WeightedBatchSelection Quality filter followed by an ambiguity reward.
...
%       A_t(x) = R~(x) + 0.50*U~(x),  ratio = FE/maxFE.
...
%   The weight is fixed at 0.50. Ratio is diagnostic metadata only.

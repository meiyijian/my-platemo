...
%--------------------------------------------------------------------------
    ...
    candidates = Next(idx(1:min(eta,end)),:);

    % 计算候选解与当前种群（Input）在决策空间的距离
    normPop = (Input - Problem.lower) ./ (Problem.upper - Problem.lower);
    normCand = (candidates - Problem.lower) ./ (Problem.upper - Problem.lower);
    dist = min(pdist2(normCand, normPop), [], 2);
    [~,maxDistIdx] = max(dist);

    % 最终选择一个最不拥挤的解进行真实评估
    Next = candidates(maxDistIdx, :);
end

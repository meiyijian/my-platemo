...
% =========================================================================
    ...
    methods
        ...
        function main(Algorithm,Problem)
            ...
            % ----- 3. 初始训练样本：拉丁超立方采样 NI = 100 个点 -----
            % 本变体固定使用 100 个真实评价样本初始化。
            NI    = 100;
            ...
            while Algorithm.NotTerminated(A2)
                ...
                PopNew    = PopNew(1:min(size(PopNew,1),max(0,floor(Problem.maxFE-Problem.FE))),:);
                New       = Problem.Evaluation(PopNew);
                % A2 单调累积所有真实评价过的解（用于评估指标）
                A2        = [A2,New];
                % A1 用于训练 Kriging：保留 NI 个解 + 新评价的 mu 个，按多样性裁剪老解
                A1        = UpdataArchive(A1,New,V,numel(New),NI);
            end
        end
    end
end

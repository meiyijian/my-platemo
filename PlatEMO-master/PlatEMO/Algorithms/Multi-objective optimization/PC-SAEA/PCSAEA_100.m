        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Arc)
                ...
                if ~isempty(Next)
                    ...
                    Next = Next(1:min(size(Next,1),max(0,floor(Problem.maxFE-Problem.FE))),:);
                    Arc = [Arc,Problem.Evaluation(Next)];
                end

                Population = EnvironmentalSelection(Arc,min(Problem.N,numel(Arc)));
                t = t+1;
            end
        end
    end

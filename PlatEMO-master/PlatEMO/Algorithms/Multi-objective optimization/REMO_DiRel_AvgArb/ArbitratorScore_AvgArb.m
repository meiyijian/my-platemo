function scores = ArbitratorScore_AvgArb(Smodel, Candidates)
% ArbitratorScore_AvgArb - 消融变体核心：用固定 0.5/0.5 简单平均
%
% 与 ArbitratorScore 的唯一差异：w_F = w_S = 0.5（不基于不确定性），冲突分支保留
    ...
    % ---- 消融差异：固定 0.5/0.5 平均 ----
    base = 0.5 .* tildeS_F + 0.5 .* tildeS_S;
...
% 复用本地化的辅助函数，与 ArbitratorScore 中实现一致

function DualNet = DR_TrainDualScaleNet(XX_F, YY_F, XX_S, YY_S, K_ens)
% DR_TrainDualScaleNet - 双尺度集成网络训练（模块②）
%
% 功能概述：
%   训练两组集成神经网络：
%   1. net_F：学习全目标（M个目标）的关系
%   2. net_S：学习易子集（k_easy个目标）的关系，并用net_F的权重做迁移初始化
%
% 为什么需要双尺度？
%   - 全目标视角：看全局，信息完整，但维度高（2M维输入），建模复杂
%   - 子目标视角：看局部，维度低（2×k_easy维输入），建模简单
%   两个视角互补：全目标看大势，子目标看细节
%
% 为什么需要集成学习？
...
%
% 调用示例：
%   DualNet = DR_TrainDualScaleNet(XX_F, YY_F, XX_S, YY_S, 3);

    % ===================================================================
    % 第一步：训练全目标集成网络 net_F
    % ===================================================================

    % DataProcess_DiRel 将数据分为训练集（75%）和测试集（25%）
    ...
    % onehotconv_DiRel 将标签转换为 one-hot 编码
    ...
    xDim_S = size(TrainIn_S_nor, 2);   % 输入维度 = 2×k_easy
...
%   nets         - 1×K cell，每个元素是一个训练好的 patternnet
    ...
    for i = 1:K
        ...
        if ~isempty(initFromNets) && i <= numel(initFromNets) && ~isempty(initFromNets{i})
            ...
            net = DR_TransferFineTune(net, initFromNets{i});
        end

...
% test_units_DR - Unit checks for REMO objective-reduction helpers.
    ...
    RefObj = UniformPoint(3, 3, 'ILD');
    [XXr, Lr, CatalogR] = DR_GetRelationPairsBudgeted(PopDec, PopObj(:, 1:3), 200, RefObj, 0.05);
    assert(size(XXr, 2) == 2 * D);
    assert(numel(Lr) == size(XXr, 1));
    assert(numel(CatalogR) == N);

    runMainText = fileread(fullfile(here, 'DR_RunMain.m'));
    assert(isempty(strfind(runMainText, 'ParameterSet')), ...
        'DR_RunMain must not call protected Algorithm.ParameterSet outside the algorithm class.');
    assert(isempty(strfind(runMainText, 'Algorithm.NotTerminated')), ...
        'DR_RunMain must not call protected Algorithm.NotTerminated outside the algorithm class.');

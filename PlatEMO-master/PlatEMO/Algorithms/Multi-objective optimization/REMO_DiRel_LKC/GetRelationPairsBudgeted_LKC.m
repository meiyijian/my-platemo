function [XXs, Ls, Catalog] = GetRelationPairsBudgeted_LKC(Input, Obj, pairMax, RefObj, scalarGap)
% GetRelationPairsBudgeted_LKC - Pareto-first relation pairs with PBI fallback.
%
% Labels are generated in the objective space provided by Obj.  For the LKC
% sub-network, Obj is the easy aggregated objective space, so the resulting
% labels must only be interpreted in that reduced space.
    ...
    N = size(Input, 1);
    D = size(Input, 2);
    Catalog = GetOutputPBI_LKC(Obj, RefObj);
...
function Catalog = GetOutputPBI_LKC(PopObj, RefObj)
% Lightweight PBI-like good/non-good catalog for anchor selection.

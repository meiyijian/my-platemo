    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Archive)
                ...
                % 学习分布信息（DISK算法的分布学习模块）
                [F,~]  = NDSort(Population.objs,inf); % 非支配排序
                PopDec = Population(F==1).decs; % 获取第一前沿解的决策变量
                if size(PopDec,1) <= 1 % 如果第一前沿解太少，加入第二前沿解
                    PopDec = [PopDec; Population(F==2).decs];
                end

function index  = EnvironmentalSelection(PopObj,PopCon,N)
...
% This function is written by Zhiyao Zhang (email: zhiyao.zhang.cn@gmail.com)
    ...
    if length(fea_index) == N
        index = fea_index;
    elseif length(fea_index) > N
        FrontNo = NDSort(PopObj(fea_index,:),PopCon(fea_index,:),1);
        index   = fea_index(FrontNo==1);
        index_d = fea_index(FrontNo~=1);
        if length(index) > N
            PObj   = PopObj(index,:);
        ...
        index    = [fea_index; infea_index(rank(1:N-length(fea_index)))];
    end
end

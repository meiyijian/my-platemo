# Pruned qKeep=0.80：删除五项参数后的算法与完整流程

> **本目录身份（2026-09-12）**：本目录是 `REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned` 的**源码同构副本**——`private/` 下 11 个共享模块字节一致，主文件除类名与头部注释外逐行相同。单独建一版是为了把 **qKeep=0.80（源码默认值）** 这组实验与 qKeep=0.70 那组实验的**数据目录和文件名前缀彻底分开**，避免互相覆盖。算法行为与 Pruned 完全一致，差异只在运行时传入的 qKeep。
>
> 上游 Pruned 的正式实验入口是**显式覆盖**成 `{3000,0.50,0.25,0.70,6}`；本变体不动默认值，入口默认即 `{3000,0.50,0.25,0.80,6}`。

## 1. 版本身份与实现范围

- 入口：`REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned_qKeep080.m`。
- 建立日期：2026-09-12；复制自 2026-09-11 建立的 Pruned 版本（其源文件身份见上游 `source_manifest.json`）。

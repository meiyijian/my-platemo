function summary = RunLambdat030_10(action,workers)
%RunLambdat030_10 Verify then run the Lambdat030 variant on four M=10 problems.
%   RunLambdat030_10('check')     audit the planned runs, no optimization.
%   RunLambdat030_10('run',W)     fill the two-arm experiment with W workers.
%   DTLZ2/4/5/7, M=10, D=30, N=100, maxFE=300, run IDs 19..28, ten runs per
%   problem and arm. Two arms share every run:
%     lambdat030 - the new variant with lambdaT=0.30,
...
%   diagnostics folder instead of the shared dataset.
    ...
    fprintf('Algorithm : %s\n',algorithm);
    fprintf('Arms      : lambdat030 {3000,0.50,0.25,0.70,6}; baseline {3000,0.50,0.25,0.70,6}\n');
...
%buildJobs One job per problem, run ID and arm, with matched seeds.
    ...
    for p = 1:numel(cfg.problems)
        ...
        for r = cfg.runIds
            ...
            arms = { ...
                'lambdat030',algorithm,cfg.root, ...
                {3000,0.50,0.25,0.70,6}, cfg.lambdaT,cfg.dataFolder; ...
            ...
            for a = 1:size(arms,1)
                job = struct('arm',arms{a,1},'algorithm',arms{a,2}, ...
                    ...
                    'lambdaT',arms{a,5},'problem',problem,'runId',r, ...
...
%runOne Evaluate one problem instance and store the result atomically.
    status = struct('ok',false,'arm',job.arm,'problem',job.problem, ...
        'runId',job.runId,'lambdaT',job.lambdaT,'IGD',NaN,'message','', ...
    ...
    try
        ...
        metadata = struct('algorithm',job.algorithm,'arm',job.arm, ...
            ...
            'modeRunId',1,'parameters',{job.parameters},'lambdaT',job.lambdaT, ...

classdef REMO_new2_AdaMaO_SimpleA < ALGORITHM
% <2026> <multi/many> <real> <expensive>
% AdaMaO-SimpleA: 消融变体（基于 AdaMaO-Simple）
%
% 本版本相对 AdaMaO-Simple 仅做一处改动：
%   - 将代理模型内部 GA 迭代预算 wmax 从 N*ceil(log2(N+1))（N=100 时=700）
%     恢复为完整版的 3000，用于验证 gmax 是否为简化版退化的元凶。
%   其余所有超参数内化、软置信权重、coverage_gap 连续控制等均与 Simple 完全一致。
%
% 与完整版相比，本版本在保持核心框架
    ...
    methods
        function main(Algorithm, Problem)
            ...
            while Algorithm.NotTerminated(Archive)
                ...
                %% ---- 代理模型辅助选择（Simple-A：仅恢复 gmax=3000）----
                % 消融验证：gmax 是否为简化版退化的元凶。此处恢复完整版的 3000，
                % 其余（k 内生化、软置信权重、coverage_gap 连续控制等）保持与 Simple 一致。
                wmax = 3000;   % 恢复完整版代理内部 GA 迭代预算（Simple 中为 N*ceil(log2(N+1))=700）
                ...
                %% ---- 调试输出（可选）----
                if debug
                    fprintf(['[AdaMaO-SimpleA Gen %3d | FE=%4d/%4d] rel=conf-weighted cand=%s ', ...

function RunQ080_18(action)
% Run four M10 problems, 18 repetitions each; each run is saved separately.
    ...
    algorithm = 'REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned_Weighted_Q080';
    ...
    logs = fullfile(root,'diagnostics','Q080_18runs');
    problems = {'DTLZ2','DTLZ5','DTLZ7','WFG7'};
    parameters = {3000,0.50,0.25,0.80,6};
    jobs = struct([]);
    for p = 1:numel(problems)
        pro = feval(problems{p},'N',100,'M',10,'D',30,'maxFE',300);
        assert(pro.D==30 && pro.M==10 && pro.N==100 && pro.maxFE==300);
        for r = 1:18
            j = struct('algorithm',algorithm,'problem',problems{p}, ...
                'runId',r,'seed',20260912+1000*p+r,'parameters',{parameters}, ...
                'file',fullfile(folder,sprintf('%s_%s_M10_D30_%d.mat',algorithm,problems{p},r)), ...
                'logs',logs);
    ...
    fprintf('Algorithm: %s\nqKeep: 0.80; M=10; D=30; N=100; maxFE=300\n',algorithm);
    ...
    assert(failures==0,'%d jobs failed. See diagnostics/Q080_18runs.',failures);
    fprintf('COMPLETE: all 72 runs saved individually.\n');
end
...
function status=runOne(job)
    ...
    try
        ...
        pro=feval(job.problem,'N',100,'M',10,'D',30,'maxFE',300);
        ...
        metadata=struct('algorithm',job.algorithm,'problem',job.problem,'runId',job.runId, ...
            ...
            'N',100,'M',10,'D',30,'maxFE',300,'save',18,'matlabVersion',version, ...

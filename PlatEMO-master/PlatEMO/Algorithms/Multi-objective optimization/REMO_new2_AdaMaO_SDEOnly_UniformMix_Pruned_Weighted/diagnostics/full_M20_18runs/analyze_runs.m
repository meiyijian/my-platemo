...
base='C:/Users/lsx/Desktop/REMOandDREMO测试集/20目标';
...
for ai=1:numel(algs)
 for pi=1:numel(problems)
  fs=dir(fullfile(base,algs{ai},sprintf('%s_%s_M20_D*_*.mat',algs{ai},problems{pi})));
  for fi=1:numel(fs)
   ...
   r.qKeep=NaN; r.seed=NaN; if strcmp(labels{ai},'Weighted'), md=load(fullfile(fs(fi).folder,fs(fi).name),'metadata'); r.qKeep=md.metadata.parameters{4}; r.seed=md.metadata.seed; end; if isfield(d.metric,'IGD'),r.IGD=d.metric.IGD(end);r.trajectoryIGD=d.metric.IGD;end

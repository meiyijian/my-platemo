function MergeIGDpForDir(alg, M, workers)
...
%   running that inside a pool has crashed MATLAB six times on this machine.
    ...
    fid = fopen(logFile,'a');
    fprintf(fid,'# MergeIGDpForDir %s M=%d workers=%d start %s\n', ...
        alg,M,workers,char(datetime('now','Format','yyyy-MM-dd HH:mm:ss')));
    ...
    for p = 1:numel(problems)
        ...
        n = numel(files);
        gp = cell(n,1);
        usePool = workers > 1 && ~(strcmp(prob,'DTLZ7') && M == 20);
        if usePool
            parfor i = 1:n
                gp{i} = TraceOne(files{i},prob,M,D);
            end
        else
            for i = 1:n
                gp{i} = TraceOne(files{i},prob,M,D);
...
% ------------------------------------------------------------------------
function gp = TraceOne(file,prob,M,D)

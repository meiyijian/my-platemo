function summary = RunLambdat030_M10_DTLZ2457(workers)
%RunLambdat030_M10_DTLZ2457 Single-arm Lambdat030 run on four M=10 problems.
%   RunLambdat030_M10_DTLZ2457(W) evaluates DTLZ2/4/5/7 with M=10, D=30,
%   N=100 and maxFE=300, using run IDs 19..28, that is ten runs per problem,
%   spread over W process workers.
%   Only the variant arm is executed, with the parameter vector
%   {3000,0.50,0.25,0.70,6} for the algorithm
%   REMO_UniformMix_Pruned_Weighted_Lambdat030. The ambiguity reward weight
%   0.30 is fixed inside the algorithm and is not a public parameter.
%   Seeds follow the published thirty-run batch stored for the same problems,
%   that is 20260912 + M*100000 + problemIndex*1000 + runId, so run IDs 19..28
%   line up with the runs already stored for the pruned baseline and the two
%   arms of the paper share the same seeds.
...
%   Existing result files are never overwritten: a run whose file is already
%   stored is skipped, which makes the runner restartable.
    ...
    for i = 1:numel(jobs)
        ...
        elapsed = toc(started);
        if s.ok
            fprintf('[%d/%d] %-10s %s run %02d IGD=%.10g | elapsed %.1f min\n', ...
...
%configuration Fixed scope of this experiment.
    ...
    cfg.dataFolder = fullfile(['C:/Users/lsx/Desktop/REMOandDREMO测试集/10目标/n30/' ...
        'REMO_UniformMix_Pruned_Weighted_Lambdat030']);
    cfg.scratch = fullfile(tempdir,'lambdat030_m10_runs');
...
%writeManifest Persist the frozen source list next to the run logs.
    text = jsonencode(struct('variantFolder',cfg.root, ...
        ...
        'seedBase',20260912,'runIds',cfg.runIds, ...
        'sources',manifest));
    fid = fopen(file,'w','n','UTF-8');

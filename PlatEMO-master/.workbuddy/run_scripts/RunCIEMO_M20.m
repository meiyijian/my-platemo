function RunCIEMO_M20(mode, workers, outOverride)
%RunCIEMO_M20  DTLZ/WFG sweep of CI-EMO (class CIEMO) at M=20.
%
%   RunCIEMO_M20('run',WORKERS)          16 problems x 20 runs
%   RunCIEMO_M20('smoke',WORKERS)        one problem (WFG9), one run, serial
%   RunCIEMO_M20('check',WORKERS)        one problem (DTLZ2), one run, serial
%   RunCIEMO_M20('probe',WORKERS)        DTLZ1/2/7 + WFG3/9, one run each
%   RunCIEMO_M20('run',WORKERS,OUTROOT)  redirect results
%
...
%     algorithm CI-EMO (class CIEMO; 2026 SWEVO, composite indicator-guided
%               infilling sampling for expensive MOO).  No 'parameter' is
%               passed; CIEMO has no ParameterSet call and derives its own
%               initial sample size (11*D-1 capped at 100).
...
%   Logs     .workbuddy\ablation_logs            (the data folder stays MAT-only)
    ...
    switch lower(mode)
        ...
        otherwise,    error('RunCIEMO_M20:Mode','Unknown mode %s', mode);
    end
end

function BuildTableM20Data()
%BuildTableM20Data Extract IGDp of the twenty-objective runs and emit the formatted
%   table texts, with the same conventions as the fifteen-objective tables:
%     - eight algorithm columns, the NoBatchDist variant last;
%     - mean and STD over runs 1-20;
%     - '%.4e (%.2e)' with 'e-0'->'e-' and 'e+0'->'e+';
%     - rank sum test against the LAST column, p<0.05, '+' better / '-' worse /
%       '=' no difference, appended as ' ' plus sign; last column carries none.
    outBase  = 'C:\Users\lsx\Desktop\AdaMao实验表';
    root     = 'C:\Users\lsx\Desktop\REMOandDREMO测试集\20目标';
    M        = 20;
    metric   = 'IGDp';
    runs     = 1:20;
    ...
    for p = 1:nP
        ...
        for a = 1:nA
            ...
            vals{a} = v(sel); means(a) = mean(vals{a}); stds(a) = std(vals{a});
        end
        ...
        [~, bi] = min(means); bestAlg{p} = labels{bi};
        for a = 1:nA-1
            switch signs(a)
    ...
    csvFile = fullfile(outBase, 'lambdat030版本', sprintf('table20_%s.csv', metric));
    writetable(T, csvFile, 'Encoding', 'UTF-8');

    fprintf('\n=== 20 objective, %s (runs 1-20, last column = NoBatchDist) ===\n', metric);
    fprintf('%-7s', 'Problem');

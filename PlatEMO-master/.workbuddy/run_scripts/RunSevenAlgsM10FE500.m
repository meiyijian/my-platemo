function summary = RunSevenAlgsM10FE500(mode,workers,onlyAlg)
%RunSevenAlgsM10FE500 Seven baselines on the sixteen-problem M=10 list, maxFE=500.
%   RunSevenAlgsM10FE500('smoke',W)  one run of DTLZ2 for every algorithm
%   RunSevenAlgsM10FE500('run',W)    full 7 x 16 x 20 = 2240 runs
%   RunSevenAlgsM10FE500('check')    report what is already stored
%   The third argument restricts the work to one algorithm, which is how the
%   batch is started algorithm by algorithm.
%
%   Fixed scope, matching the rest of the paper's M=10 batches:
%   M=10, D=30, N=100, maxFE=500, save=30, run numbers 1..20,
%   seed = 20260912 + M*100000 + problemPosition*1000 + runId  (M=10 -> 21760912).
...
%   because the PlatEMO helpers share file names across algorithm folders.
    ...
    if failures > 0
        error('M10FE500:RunsFailed','%d job(s) failed. See %s.',failures,cfg.scratch);
    end
end
...
%configuration Fixed scope of this experiment.
    ...
    cfg.dataFolder = fullfile(cfg.testRoot,'10目标','n30','FE500');
    cfg.scratch = fullfile(tempdir,'m10_fe500');
...
%runOne Evaluate one problem instance and store the result atomically.
    ...
    try
        ...
        if isfile(job.file)
            error('M10FE500:ResultAppeared','Result now exists; not overwriting.');
        end

function summary = RunLambda020ArmA(workers)
%RunLambda020ArmA Run the lambda020 arm only on four M=10 problems.
%   RunLambda020ArmA(W) evaluates DTLZ2/4/5/7 with M=10, D=30, N=100 and
%   maxFE=300, using run IDs 19..28, that is ten runs per problem, spread
%   over W process workers.
...
%   already stored is skipped, which makes the runner restartable.
    ...
    fprintf('Problems  : %s | M=%d D=%d N=%d maxFE=%d\n', ...
        strjoin(cfg.problems,', '),cfg.M,cfg.D,cfg.N,cfg.maxFE);
    fprintf('Dataset   : %s\n',cfg.dataFolder);
    fprintf('Scratch   : %s\n',cfg.scratch);
...
%configuration Fixed scope of this experiment.
    ...
    cfg.dataFolder = fullfile(['C:/Users/lsx/Desktop/REMOandDREMO测试集/10目标/n30/' ...
        'REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned_Weighted_Lambda020']);
    cfg.scratch = fullfile(tempdir,'lambda020_armA_runs');

function summary = RunLambdat030_M10_Full(workers)
%RunLambdat030_M10_Full Fill the M=10 batch of the Lambdat030 variant.
%   RunLambdat030_M10_Full(W) evaluates the full M=10 problem list of the
%   paper, that is DTLZ1..DTLZ7 followed by WFG1..WFG9, with twenty runs per
%   problem, at D=30, N=100 and maxFE=300, spread over W process workers.
%   The runs are submitted DTLZ first and WFG second, so the DTLZ set finishes
%   before the WFG set starts.
%   Seeds follow the published batch of the sixteen-problem list,
%   20260912 + M*100000 + problemPosition*1000 + seedId, where seedId is the
%   run number plus the per-problem offset below. Ten runs of DTLZ2/4/5/7 were
%   already produced in this same session and now occupy the run numbers 1..10
%   with seedId 19..28; their offset 18 therefore makes run number 11..20
%   continue at seedId 29..38, so no seed is repeated. Every other problem has
%   offset 0 and uses seedId 1..20.
%   Result files land in the dataset folder of this algorithm as .mat files.
%   Every other product, such as the source manifest, the per-round records
%   and the failure reports, goes to the system temporary directory, so the
%   dataset folder ends up holding result files and nothing else.
%   Existing result files are never overwritten: a stored run is skipped,
%   which makes the runner restartable.

    if nargin < 1, workers = 'max'; end
    ...
    fprintf('Algorithm : %s\n',algorithm);
    fprintf('Arm       : lambdat030 {3000,0.50,0.25,0.70,6} with lambdaT=0.30 fixed\n');
    ...
    for i = 1:numel(jobs)
        ...
        rate = elapsed/i;
        if s.ok
            fprintf('[%3d/%3d] %-9s run %02d IGD=%.10g | %.1f min elapsed, %.2f min/run, eta %.1f min\n', ...
                i,numel(jobs),s.problem,s.runId,s.IGD,elapsed/60,rate/60, ...
                (numel(jobs)-i)*rate/60);
        else
            failures = failures + 1;
...
%configuration Fixed scope of this experiment.
    ...
    cfg.dataFolder = fullfile(['C:/Users/lsx/Desktop/REMOandDREMO测试集/10目标/n30/' ...
        'REMO_UniformMix_Pruned_Weighted_Lambdat030']);
    cfg.scratch = fullfile(tempdir,'lambdat030_m10_full');
    cfg.recordsFolder = fullfile(cfg.scratch,'records');
    % Position inside the paper's sixteen-problem list: DTLZ1..DTLZ7 are the
    % first seven, WFG1..WFG9 the last nine. DTLZ first in the job order.
    cfg.problems = {'DTLZ1','DTLZ2','DTLZ3','DTLZ4','DTLZ5','DTLZ6','DTLZ7', ...
        'WFG1','WFG2','WFG3','WFG4','WFG5','WFG6','WFG7','WFG8','WFG9'};
    cfg.problemIndex = 1:16;
    cfg.indices = 1:20;
    % Per-problem seed offset. DTLZ2/4/5/7 already hold runs 1..10 with seedId
    % 19..28, so run number n maps to seedId n+18 and the remaining ten runs
    % use seedId 29..38 without repeating a seed. All other problems start at 1.
    cfg.seedOffset = [0 18 0 18 18 0 18, 0 0 0 0 0 0 0 0 0];
    ...
    cfg.maxFE = 300;
    cfg.save = 30;
    % Helpers of this algorithm that share their file name with other
    % algorithm folders on the global path. They must resolve to this folder,
    % otherwise a foreign copy would silently be executed.
    cfg.sharedHelpers = {'ResolveUniformMixMode','PrunedIndicatorSelection', ...
        'PrunedWeightedBatchSelection','Lambdat030WeightedBatchSelection'};
    cfg.privateHelpers = {'DiversifiedInfillSelection','IndicatorSelectorSDEOnly', ...
        'PBIQualityClassification','CreateSDECandidateModeStream'};
end

function assertHelpersResolveTo(cfg)
%assertHelpersResolveTo Fail fast when a shared helper resolves elsewhere.
    for i = 1:numel(cfg.sharedHelpers)
        file = which(cfg.sharedHelpers{i});
        assert(~isempty(file),'Missing helper %s.',cfg.sharedHelpers{i});
        assert(strcmpi(fileparts(file),cfg.root), ...
            'Helper %s resolves to %s instead of %s.', ...
            cfg.sharedHelpers{i},fileparts(file),cfg.root);
    end
    % Private helpers cannot be located with WHICH from outside their folder,
    % they are visible to the algorithm folder only. A private copy always wins
    % over a same-named file elsewhere on the path, so existence is enough.
    for i = 1:numel(cfg.privateHelpers)
        file = fullfile(cfg.root,'private',[cfg.privateHelpers{i},'.m']);
        assert(isfile(file),'Missing helper %s.',file);
    end
end

function jobs = buildJobs(cfg,algorithm,manifestFile)
%buildJobs One job per problem and run number of the single variant arm.
    jobs = struct([]);
    for p = 1:numel(cfg.problems)
        problem = cfg.problems{p};
        pro = feval(problem,'N',cfg.N,'M',cfg.M,'D',cfg.D,'maxFE',cfg.maxFE);
        % WFG2 and WFG3 raise D to the next valid value themselves, so the
        % stored file name uses the D the problem reports, not the request.
        assert(pro.M==cfg.M && pro.N==cfg.N && pro.D>=cfg.D, ...
            'Unexpected %s configuration.',problem);
        for r = cfg.indices
            seed = 20260912 + cfg.M*100000 + cfg.problemIndex(p)*1000 + ...
                (r + cfg.seedOffset(p));
            job = struct('arm','lambdat030','algorithm',algorithm, ...
                'algFolder',cfg.root,'parameters',{cfg.parameters}, ...
                'problem',problem,'runId',r,'seed',seed, ...
                'root',cfg.root,'platform',cfg.platform, ...
...
%runOne Evaluate one problem instance and store the result atomically.
    status = struct('ok',false,'arm',job.arm,'problem',job.problem, ...
        'runId',job.runId,'IGD',NaN,'message','', ...
        'file',job.file,'exploreRounds',0,'meanLambdaT',NaN, ...
        'setChangedShare',NaN);
    previousPath = path;
    cleanup = onCleanup(@()path(previousPath)); %#ok<NASGU>
    global ADAMAO_LAMBDAT030_DIAG %#ok<GVMIS>
    ADAMAO_LAMBDAT030_DIAG = struct('enabled',false);
    temporary = '';
    try
        ...
        assert(strcmpi(fileparts(which('ResolveUniformMixMode')),job.algFolder), ...
            'A foreign ResolveUniformMixMode shadowed this algorithm.');
        assert(strcmpi(fileparts(which('Lambdat030WeightedBatchSelection')), ...
            job.algFolder),'A foreign batch selector shadowed this algorithm.');
        ...
        metadata = struct('algorithm',job.algorithm,'arm',job.arm, ...
            ...
            'modeRunId',1,'parameters',{job.parameters},'lambdaT',0.30, ...
        ...
        assert(isstruct(ADAMAO_LAMBDAT030_DIAG) && ...
            isfield(ADAMAO_LAMBDAT030_DIAG,'rounds') && ...
            ADAMAO_LAMBDAT030_DIAG.rounds > 0, ...
            'Diagnostics were not collected.');
        diagnostics = summarizeDiagnostics(ADAMAO_LAMBDAT030_DIAG);
        assert(diagnostics.lambdaT == 0.30,'Unexpected reward weight.');
        ...
        assert(ok,'%s',message);
        temporary = '';
        writeRecords(job,ADAMAO_LAMBDAT030_DIAG);
...
%summarizeDiagnostics Compact per-run summary of the exploration records.
    ...
    n = numel(records);
    diagnostics = struct('lambdaT',DIAG.lambdaT,'rounds',DIAG.rounds, ...
...
%writeManifest Persist the frozen source list next to the run logs.
    text = jsonencode(struct('variantFolder',cfg.root, ...
        ...
        'seedBase',20260912,'seedOffset',cfg.seedOffset, ...
        'indices',cfg.indices,'sources',manifest));
    fid = fopen(file,'w','n','UTF-8');

function summary = RunLambdat030_M15_Full(workers)
%RunLambdat030_M15_Full Run the fifteen-objective batch of the Lambdat030 arm.
%   RunLambdat030_M15_Full(W) evaluates the full problem list of the paper at
%   M=15, that is DTLZ1..DTLZ7 followed by WFG1..WFG9, with twenty runs per
...
%   Seeds follow the same published rule as every other objective count,
%   20260912 + M*100000 + problemPosition*1000 + runId, with M=15 and runId
%   1..20. No result of this arm exists yet, so there is no offset: the stored
%   file number is the runId. The rule is the one already used by the stored
%   baseline of this dataset, whose DTLZ1 run 1 carries seed 21761913.
...
%configuration Fixed scope of this experiment.
    ...
    % The fifteen-objective dataset keeps one folder per algorithm directly
    % under the objective folder, with no intermediate size folder.
    cfg.dataFolder = fullfile(['C:/Users/lsx/Desktop/REMOandDREMO测试集/15目标/' ...
        'REMO_UniformMix_Pruned_Weighted_Lambdat030']);
    cfg.scratch = fullfile(tempdir,'lambdat030_m15_full');
...
%buildJobs One job per problem and run number of the single variant arm.
    ...
    for p = 1:numel(cfg.problems)
        ...
        % Some WFG problems raise D to the next valid value themselves, so the
...
%writeManifest Persist the frozen source list next to the run logs.
    text = jsonencode(struct('variantFolder',cfg.root, ...
        ...
        'seedBase',20260912,'indices',cfg.indices,'sources',manifest));
    fid = fopen(file,'w','n','UTF-8');

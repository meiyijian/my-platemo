...
Smaller IGD is better; bold highlights the lowest mean in each row. The supplied PlatEMO export marks each non-default setting as better than ($+$), worse than ($-$), or without a detected difference from ($=$) $p_{\mathrm{mix}}=0.5$. We compute average ranks from the mean IGD values within each objective setting.
\end{minipage}
\end{table*}

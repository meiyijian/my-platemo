...
The component ablation uses the complete PACDIS as Full and compares it with
...
CDIS selection rule. The CDIS control retains PAQC and uses a
predefined relation-only candidate rule under the same batch cap.
These supplied ablations use a 300-evaluation budget and are separate from
the 500-evaluation main comparison.

Tables~\ref{tab:ablation_igdplus_dtlz} and~\ref{tab:ablation_igdplus_wfg}
report the supplied IGD$^+$ results for the two deletion controls and REMO
with $k=15$ and 30 at $M=10$ and 20, respectively.
The tables retain the reported means, standard deviations, and significance
labels, with Full as the reference. Across both objective counts, the labels
favor Full over the CDIS control on 12 of 32 instances and over the PAQC
control on 10 of 32 instances. The reverse comparisons favor the controls
on three and two instances, respectively. Removing CDIS improves the reported
results on DTLZ6 at both objective counts and DTLZ7 at $M=10$; removing PAQC
improves WFG3 at both objective counts. The component comparisons therefore
suggest complementary, problem-dependent benefits at the two interfaces.
The similar counts of favorable comparisons do not establish equal effect
sizes or a statistical interaction between the components.

\input{experiments/ablation/table_ablation_igdp}

\TODO{Complete the provenance audit for the supplied ablation tables:
confirm the mapping to the deletion controls above, run counts, replacement
boundaries, source hashes, seeds, thread settings, actual terminal FE,
and the statistical test and significance threshold.}

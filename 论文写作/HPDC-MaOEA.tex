...
coarse membership signal supplied by the binary PBI criterion and the dependence of
...
label used in REMO.
The resulting hybrid score prioritizes solutions before discretization,
refining positive-group membership for relation learning. Second,
...
\While{$FE<FE_{\max}$}
  ...
  \State $\mathcal{S}\gets$ \Call{CDIS}{$\mathcal{P},\mathcal{R},\mathcal{M}_R,\mathcal{M}_I,m,t,e_r$}
...
\Else
  \State Compute exploration scores $A_{\mathrm{exp}}$ by \eqref{eq:expscore}--\eqref{eq:lambda}
  \State $\mathcal{H}^{\mathrm{exp}}\gets$ candidates retained by \eqref{eq:prefilter-exp}
  \State Seed $\mathcal{S}$ with the highest-scoring member of $\mathcal{H}^{\mathrm{exp}}$
  \State Greedily extend $\mathcal{S}$ to $n_b$ members by \eqref{eq:greedy}
\EndIf
...
\TODO{Reinforce the two design contributions of PACDIS: PAQC refines

...
%% Two-column width: the \qquad-separated pair of the single-column draft is
%% set as two aligned lines. Symbols and wording are unchanged.
\begin{equation}
\begin{split}
A(\vx\mid\mathcal{S})
...
%% Two-column width: the condition text is broken over two lines. The words
%% and their order are unchanged.
\begin{equation}
m_t=
\begin{cases}
\mathrm{I}, & \text{if the indicator model is available}\\
            & \text{and }\xi_t<p_{\mathrm{mix}},\\[2pt]
...
%% The parameter table spans both columns, so it is declared here in order to
%% be placed at the top of a page close to its citation at the end of this
%% subsection. Only the float position in the source moved; the table itself
%% is unchanged.
\begin{table*}[t]
\centering

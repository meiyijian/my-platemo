...
The ablation considers four configurations with the representative count
fixed at $k=1.5M$: REMO, REMO+PAQC, REMO+CDIS, and Full (PACDIS).
REMO provides the reference grouping and selection rules. REMO+PAQC
changes quality classification, REMO+CDIS changes infill selection, and
Full combines both modules. Thus, the requested representative count is
15 at $M=10$ and 30 at $M=20$ for every configuration.

The available results appear in Table~\ref{tab:exp:ablation}.
REMO+CDIS remains to be completed. The exports specify
$N=100$ and a configured budget of 300 evaluations. The decision dimension
is 30, except for WFG2 and WFG3, which use 31. All 16 problems are retained
at $M=10$. At $M=20$, WFG4 is excluded because its exported objective
count is 10, leaving 15 problems.

\input{experiments/ablation_original/table_fixed_k}

With the same reported $k$, REMO+PAQC achieves lower mean IGD than REMO
on 14 of 16 problems at $M=10$ and 11 of 15 at $M=20$. The corresponding
pairwise outcomes in the source exports are 8/1/7 and 10/2/3
(better/worse/no detected difference). These comparisons suggest a benefit
from PAQC when the representative count is fixed. The benefit is not
uniform: REMO+PAQC has a higher mean than REMO on WFG3 at both objective
counts and on DTLZ2 at $M=20$.

Adding CDIS to REMO+PAQC produces a smaller, problem-dependent gain.
Full improves mean IGD on 10 of 16 and 9 of 15 problems, with three
favorable and one unfavorable exported pairwise outcome at each objective
count. DTLZ7 shows the largest relative improvement in this comparison:
mean IGD decreases from 17.305 to 12.122 at $M=10$ and from 25.637 to
18.154 at $M=20$, reductions of 30.0\% and 29.2\%. In contrast, Full
performs worse on DTLZ5 at both objective counts, while the WFG6 means
at $M=20$ are nearly unchanged (19.109 and 19.097).

The available comparisons therefore support PAQC under the reference
selection rule and a conditional gain from CDIS when PAQC is present.
The REMO+CDIS configuration is needed to examine CDIS without PAQC and
PAQC with CDIS. These results do not yet establish an interaction between
the two modules.

\TODO{Complete REMO+CDIS at $k=1.5M$. Confirm implementation and parameter
alignment, run counts, actual terminal FE, IGD reference sets and the
statistical-test settings from individual-run records before finalizing
the component conclusions.}

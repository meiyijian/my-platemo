...
%  Implementation: REMO_new2_AdaMaO_SDEOnly_UniformMix_Pruned_Weighted.
...
\texttt{REMO\_\allowbreak new2\_\allowbreak AdaMaO\_\allowbreak SDEOnly\_\allowbreak UniformMix\_\allowbreak Pruned\_\allowbreak Weighted}.
...
on 3, 4 and 4 of the 16 problems at $M=10$, 15 and 20, respectively.
These best means occur on WFG1, WFG7 and WFG8 at all three objective counts,
and additionally on WFG6 at 15 and 20 objectives.

\input{experiments/main_performance/table_dtlz}
\input{experiments/main_performance/table_wfg}

The REMO columns have $+/-/=$ counts of 5/10/1, 6/10/0 and 4/9/3
...
7/5/4, 8/6/2 and 6/6/4. Thus, the results indicate competitive performance
...
\subsection{Factorial Ablation of PAQC and CDIS}
\label{sec:exp:ablation}

The component study is designed to separate the contribution of PAQC from
the supporting CDIS strategy. Four variants share the same implementation
framework: $A_{00}$ uses the reference grouping and selection rules;
$A_{10}$ replaces only grouping with PAQC; $A_{01}$ replaces only selection
with the simplified CDIS; and $A_{11}$ uses both modules and is the complete
PACDIS. The reference rules are controlled replacements within this common
framework, rather than a claim to reproduce the complete original REMO.
\begin{table}[t]
\centering
\caption{Planned factorial component configurations.}
...
\midrule
$A_{00}$ & Reference & Reference\\
$A_{10}$ & PAQC & Reference\\
$A_{01}$ & Reference & CDIS\\
$A_{11}$ & PAQC & CDIS\\
\bottomrule
\end{tabular}
\end{table}
All four variants will use matched initial designs, real-evaluation budgets,
representative-count rules, relation-model training, and environmental
selection. The exact reference-selection rule and its batch policy will be
specified before running the controls; any change in evaluation-batch size
must be reported as part of that treatment. The PAQC contrasts are
$A_{10}$ versus $A_{00}$ and $A_{11}$ versus $A_{01}$, while the CDIS
contrasts are $A_{01}$ versus $A_{00}$ and $A_{11}$ versus $A_{10}$.
Their consistency will distinguish a PAQC benefit across selection contexts
from a benefit conditional on CDIS.
\TODO{Insert the fixed problem subset, run count, seeds, exact reference
rules and batch policy, final-IGD results, effect sizes and statistical
comparisons. No component-performance conclusion is asserted yet.}

\subsection{Control for the Number of Representative Solutions}
\label{sec:exp:kcontrol}

This control tests whether an apparent PAQC benefit can instead be explained
by using more representatives. With CDIS fixed, reference grouping and PAQC
will each be evaluated at $k=6$ and $k=k_{\mathrm{eff}}$ from
\eqref{eq:keff}. Grouping methods will be compared at the same $k$, and
the effect of changing $k$ will be assessed within each grouping method.
Representatives are reused in the mating pool, so changing $k$ can affect
candidate generation as well as grouping; the actual returned count and
candidate-generation accounting will therefore be recorded. Identical
configurations shared with the factorial study can reuse the same runs.
\TODO{Insert the four-configuration results, actual representative counts,
matched-run details and uncertainty estimates. Assess whether PAQC retains
a benefit at fixed $k$ before excluding representative count as an explanation.}

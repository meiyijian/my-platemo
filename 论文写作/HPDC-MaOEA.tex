...
from the current nondominated set and converting the resulting
...
direction set $\mathcal{V}=\{\vv_j\}$. In the considered many-objective
setting, directions are constructed from the current nondominated solutions
as radial unit vectors,
\begin{equation}
...
follow the current nondominated distribution.
...
The diagram uses two objectives to visualise the many-objective method.
...
relation model is trained on. Early in the search, continuous quality receives more weight in group
construction; as the budget is consumed, representative-based membership
becomes dominant. Whether this weighting produces better positive groups
is to be evaluated by the group-quality analysis in Section~\ref{sec:exp:ggp}.

...
objective vectors and uses their continuous PBI scores to supplement
...
possible within-batch concentration under relation-score-only selection; PAQC as the
...
PAQC supplements the representative-based grouping signal with adaptive
directions constructed directly from current nondominated objective vectors.
Continuous PBI assessment along these directions introduces distribution
information that does not pass through the two-dimensional radial grid.
The resulting quality scores are fused with the original binary labels to
construct the positive training group while retaining REMO's three-class
relation-learning scheme. This design aims to complement the information
supplied by the representatives; it does not reconstruct the projected geometry.
...
set. Explain how continuous PBI scores and the original binary labels jointly
...
grouping signal induced by radially selected representatives with continuous
PBI quality information from directions constructed directly from the current
nondominated set in the original objective space. The two signals
...
Relation learning requires training groups constructed from evaluated solutions.
In REMO~\cite{ref:remo}, these groups are induced by representatives selected
through RSEA's two-dimensional radial grid~\cite{ref:rsea}. Although the
selected representatives retain their original objective vectors, their
selection is guided by the projected distribution. Their induced directions
may therefore leave some structure of the current high-dimensional
nondominated distribution insufficiently represented.

PAQC complements this information by constructing adaptive directions directly
from the current nondominated objective vectors and assessing the evaluated
population with continuous PBI scores along those directions. This additional
assessment bypasses the two-dimensional selection grid. A stage-dependent
fusion of these scores and the representative-based binary labels selects a
prescribed fraction of the population as the positive group; the remaining
solutions form its complement. The representative-based grouping signal and
the three-class relation-label scheme are retained.
...
provides the grouping signal induced by the radially selected representatives,
while the adaptive-direction score adds quality information from the current
nondominated distribution in the original objective space. This signal is derived from a small set
...
reference solutions, while the adaptive-direction score adds a continuous
quality assessment based directly on the current nondominated distribution. The two signals are combined by a
...
PAQC supplements representative-based grouping with continuous PBI information
...
high-dimensional objective space. The additional information is used to form

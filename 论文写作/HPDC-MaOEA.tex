...
\newcommand{\bestcell}{\cellcolor{black!15}}
...
complementary in design. The REMO-referenced component comparisons report
problem-dependent complete-configuration outcomes. Direct module effects
require Full-versus-control tests. Lead the results
...
any direct module tests in the experimental section.}
\end{abstract}
...
algorithms, while REMO-referenced component comparisons place the complete
method beside its PAQC and CDIS deletion controls. PACDIS shows competitive
performance on the tested settings, with
consistent mean IGD$^+$ advantages on WFG2 and WFG7 across the three objective
counts; HES-EA remains stronger on DTLZ5 and DTLZ7. The REMO-referenced
comparisons favor PACDIS on 28 of 32 tested configurations. These exports
lack direct module comparisons. Group-quality
...
$\min_{\vx\in\Omega}\vf(\vx)=[f_1(\vx),\ldots,f_M(\vx)]$,
where $\Omega\subseteq\mathbb{R}^{D}$ and every evaluation of $\vf$ is
expensive. The number of real function evaluations is therefore limited to a
budget $FE_{\max}$. Let $FE$ be the number of
...
\vf_i^{\mathsf T}\vv_j/(\|\vf_i\|_2\|\vv_j\|_2)$. The conventional
PBI value~\cite{ref:moead} and its bounded PAQC quality signal are
\begin{equation}
\begin{aligned}
...
\label{eq:pbi}
\end{equation}
where $\theta$ is the penalty parameter. A larger $S_i$ indicates a smaller
PBI value along the associated direction.
...
quality information from the original objective space. With
$t=FE/FE_{\max}$, PAQC defines the hybrid quality score and the two groups as
\begin{equation}
\begin{aligned}
H_i&=(1-t)S_i+tL_i,\\
\mathcal{C}_1&=\bigl\{\vx_i\in\mathcal{P}\;\big|\;
  \operatorname{rank}_{\downarrow}(H_i)\leq\lceil r_gN\rceil\bigr\},\\
\mathcal{C}_2&=\mathcal{P}\setminus\mathcal{C}_1 .
\end{aligned}
\label{eq:group}
\end{equation}
With $r_g<1/2$, the compact positive group concentrates pairwise supervision on
the highest-ranked fraction of the population, while $\mathcal{C}_2$ provides a
broad contrast set.
...
\eqref{eq:group}. PAQC therefore modifies the ranking used to form
...
\State Compute directional quality signal $S_i$ by \eqref{eq:pbi}
...
By changing membership of $\mathcal{C}_1$, the fusion changes the ordered pairs
used to train the relation model. As $FE$ increases, group construction shifts
from directional quality toward representative-based membership; the
group-quality analysis in Section~\ref{sec:exp:ggp} examines the resulting
positive-group quality.
...
Candidates whose relation scores are at or above the
$q_{\mathrm{keep}}$-quantile of $R(\mathcal{A})$ form
$\mathcal{H}^{\mathrm{amb}}$, with $q_{\mathrm{keep}}=0.70$. In the absence of
ties, this screen retains the highest-scoring 30\% of the pool; ties can
increase that fraction. Ambiguity is considered only after this quality
screen, so a candidate cannot enter the batch solely because of high predicted
ambiguity.

To distinguish candidates within the retained set, let $\mathcal{O}(\vx)$
denote all ordered comparisons between candidate $\vx$ and the positive and
non-positive training groups. Let $\pi_k(\vu,\vv)$ be the predicted
probability of relation class $k\in\{-1,0,+1\}$ for an ordered pair
$(\vu,\vv)$. The candidate-level ambiguity and the acquisition score within
$\mathcal{H}^{\mathrm{amb}}$ are
\begin{equation}
\begin{aligned}
U(\vx)&=1-\frac{1}{|\mathcal{O}(\vx)|}
  \sum_{(\vu,\vv)\in\mathcal{O}(\vx)}
  \max_{k\in\{-1,0,+1\}}\pi_k(\vu,\vv),\\
A_{\mathrm{amb}}(\vx)&=\widetilde{R}(\vx)+\lambda\,\widetilde{U}(\vx),
  \qquad \lambda=0.30.
\end{aligned}
\label{eq:expscore}
\end{equation}
Here tildes denote min--max normalization within
$\mathcal{H}^{\mathrm{amb}}$, with a constant quantity assigned the normalized
value $0.5$. Thus, ambiguity refines the relation-score ranking only among
candidates that pass the quality screen. The ambiguity-oriented branch returns
the first $n_b$ candidates in descending $A_{\mathrm{amb}}$ order, preserving
...
indicator $I_i$ for each solution in the current population and train an
RBF-SVR, denoted by $\mathcal{M}_I$, on
$\mathcal{D}=\{(\vx_i,I_i)\}_{i=1}^{N}$; its prediction is written
...
surrogate is unavailable. Let $\mathcal{H}$ denote the retained set selected by
the current mode. The feasible batch size is
$n_b=\min(n_{\max},|\mathcal{H}|,FE_{\max}-FE)$, with $n_{\max}=6$.
A small retained set produces a smaller batch; it is not
...
\If{$m=\mathrm{ind}$}
  \State $\mathcal{H}\gets$ top $q_{\mathrm{rel}}$ fraction by $R$
  \State Rerank $\mathcal{H}$ by $\widehat I$, falling back to $R$ if needed
\Else
  \State $\mathcal{H}\gets\{\,\vx\in\mathcal{A}:R(\vx)\geq Q_{q_{\mathrm{keep}}}(R(\mathcal{A}))\,\}$
  \State Compute $A_{\mathrm{amb}}$ within $\mathcal{H}$ by \eqref{eq:expscore}
\EndIf
\State $n_b\gets\min(n_{\max},|\mathcal{H}|,FE_{\max}-FE)$
\State $\mathcal{S}\gets$ the first $n_b$ candidates in $\mathcal{H}$ under the selected ranking
...
\eqref{eq:pbi} cost $O(NN_vM)$, the direction set
...
predictions on the screened subset. Sorting the ambiguity-rewarded acquisition
costs $O(N_c\log N_c)$.
...
among the seven retained algorithms has a light gray background. The pairwise
statistical annotations come from the supplied workbooks. A $+$ indicates
that the baseline was reported as better than PACDIS, a $-$ indicates that it
was reported as worse, and an $=$ indicates no detected difference. The bottom
row of each table counts these symbols in the order $+/-/=$ for each baseline
across all problem--objective configurations in that table. These counts
summarize the supplied pairwise outcomes and are not a separate omnibus test
across problems.
...
\clearpage
\subsection{REMO-Referenced Component Comparisons}
\label{sec:exp:ablation}

We compare the current PACDIS implementation (Full), two deletion controls,
and REMO. The w/o PAQC control uses REMO's representative-based PBI
classification in place of PAQC's fused group construction. The REMO
classifier adapts its PBI threshold, giving a variable positive-group size;
Full fixes this size at 25\%. This control retains the indicator model and
criterion switching.

The w/o CDIS control retains PAQC but removes the SDE indicator model,
indicator criterion, and criterion switching. Every round uses relation-score
quantile screening followed by ambiguity-rewarded ranking. This control thus
tests the indicator criterion and switching while retaining the ambiguity
criterion. Full and both controls use the same relation-model architecture,
representative-selection routine, candidate-generation threshold,
environmental selection, batch-size cap, and absence of within-batch distance.

The supplied IGD$^+$ workbooks cover DTLZ1--7 and WFG1--9 at $M=10$ and 20,
with $N=100$ and a configured 300-evaluation cap that includes initialization.
Full and the two controls each have 20 runs per configuration and end at 300
evaluations. REMO uses $k=15$ and 30
at $M=10$ and 20, respectively; its archived runs can exceed 300 actual
evaluations. The workbooks give mean, standard deviation, and comparison
symbols relative to REMO. We retain those symbols without reconstructing tests
from aggregate values. The workbooks omit Full-versus-control tests and any
record of multiple-comparison adjustment. This study uses a separate
configured budget from the 500-evaluation main comparison.

Tables~\ref{tab:ablation_igdplus_dtlz} and~\ref{tab:ablation_igdplus_wfg}
report the results. Relative to REMO, the workbooks mark Full as better on
28 of 32 configurations, worse on WFG3 at both objective counts, and $=$
on WFG5 at both counts. The corresponding $+/-/=$
totals are $19/2/11$ for w/o CDIS and $20/1/11$ for w/o PAQC. The controls
also differ from Full in their means: w/o CDIS has a lower mean on DTLZ6 at
both objective counts and DTLZ7 at $M=10$. These REMO-referenced annotations
describe the four complete configurations. Module attribution requires
direct Full-versus-control tests. REMO's possible budget overrun further
limits attribution.
...
% Queue the NoBatchDist sensitivity table early for two-column placement.
\input{experiments/pmix_nobatchdist_igdp_table}
...
Table~\ref{tab:exp:pmix} compares five settings in the NoBatchDist
configuration on six problems at each of $M=10$ and 20. Each cell uses
20 runs and reports final IGD$^+$. Across the 12 problem--objective
configurations, $p_{\mathrm{mix}}=0.50$ has the lowest average rank, 2.08;
$p_{\mathrm{mix}}=0.75$ follows at 2.33. The default does not lead on every
problem: $p_{\mathrm{mix}}=0$ improves DTLZ7 at $M=10$, and
$p_{\mathrm{mix}}=0.25$ improves DTLZ7 at $M=20$, according to the paired
tests in the source table. At $p_{\mathrm{mix}}=1$, the algorithm can still
use the ambiguity-oriented mode when the indicator surrogate is unavailable.
The results support $0.50$ as the best-ranked setting in this tested subset.
...
screening. The REMO-referenced comparisons favor PACDIS on most tested
configurations, with reversals on WFG3. The exported symbols compare each
configuration with REMO. Quantifying either module's conditional contribution
requires direct Full-versus-control results. PAQC and CDIS address complementary
interfaces by design; their respective effects on final IGD$^+$ require

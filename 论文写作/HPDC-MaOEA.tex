...
representative-based grouping relies on a small set of selected reference
solutions and does not explicitly exploit the directional distribution
represented by the current nondominated set,
...
PAQC complements representative-based grouping by introducing a
population-adaptive directional assessment and converting it into a quality
priority for positive-group construction. CDIS provides a supporting
...
Representative-based grouping in REMO relies on a small set of selected
reference solutions and does not explicitly exploit the directional distribution
represented by the current nondominated set. PAQC complements this
representative-based information by introducing a population-adaptive
directional assessment derived from the current nondominated set and converting
it into a quality priority for positive-group construction. The two signals are
fused to determine training-group membership, retaining the three-class
relation-learning scheme. To support the use of these predictions,
...
representative-based binary grouping signal with a continuous direction-aware
quality priority derived from population-adaptive directions. The two signals
are fused to determine which solutions enter the positive training group.
...
REMO~\cite{ref:remo} provides a coarse membership signal, but
representative-based grouping relies on a small set of selected reference
solutions and does not explicitly exploit the directional distribution
represented by the current nondominated set. PAQC complements this
representative-based information by constructing population-adaptive directions,
where applicable, from the current nondominated set and converting the resulting
directional assessment into a quality priority for positive-group construction.
A stage-dependent fusion of this priority and the representative-based binary
label selects a prescribed fraction of the population as the positive group;
the remaining solutions form its complement. This construction refines
positive-group membership while retaining the same three-class relation-label
scheme.
...
The binary label produced by \textsc{RepresentativeBasedClassification}
provides a coarse membership signal induced by a small set of representative
solutions, while the adaptive-direction score supplies an additional continuous
quality priority. This signal is derived from a small set
...
The representative-based label provides a coarse membership signal within
reference-associated regions, while the adaptive-direction score provides a
continuous direction-aware quality priority. The two signals are combined by a
weight that follows the consumption of the expensive budget. With
...
Part~(i) shows that, within the same representative-based label class, the
direction-aware quality priority $S_i$ refines the ordering used to fill the
positive-group quota when the cutoff falls inside that label class.

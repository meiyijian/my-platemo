...
This study examines PAQC and CDIS through the available component
comparisons. PAQC changes quality classification, CDIS changes infill
selection, and Full combines both modules. The configuration names follow
the terminology in the method section.
...
\begin{table}[t]
\centering
\caption{Component configurations for the PAQC and CDIS study.}
\label{tab:exp:ablation-design}
\small
\begin{tabular}{llcc}
\toprule
Label & Configuration & $k$ & Available $M$\\
\midrule
R6 & REMO & 6 & 10, 20\\
RK & REMO ($k=K$) & $K$ & 10, 20\\
P6 & PAQC only & 6 & 10, 20\\
PK & PAQC only ($k=K$) & $K$ & 10, 20\\
C6 & CDIS only & 6 & 10, 20\\
FK & Full ($k=K$) & $K$ & 10, 20\\
F6 & Full ($k=6$) & 6 & 20\\
\bottomrule
\end{tabular}
...
mean ratios are 1.063 and 1.118. Thus, the PAQC-only configuration
...
CDIS therefore gives a problem-dependent incremental gain in these
comparisons.
...
CDIS-only results at $k=K$ and Full at $M=10$, $k=6$ are absent
from the supplied exports.

\TODO{Confirm implementation and parameter alignment across configurations
before attributing these results to the current method. Verify run counts,
seeds, actual terminal FE, runtime
...
count $K$ within REMO, PAQC-only and Full configurations. REMO and
PAQC-only use the reference selection strategy, while Full includes CDIS.
The Full comparison is available only at $M=20$. These configurations
do not yet provide the complete grouping-by-$k$ control with CDIS fixed.
...
for PAQC only, whereas the corresponding WFG ratios are 1.121 and 1.143.
...
For example, increasing $k$ in PAQC only changes mean IGD on 20-objective
...
The aggregate results reinforce this point. For PAQC only at $M=20$, the
